	...
	$.post("<%=BaseAction.rootLocation %>/parkmanager.oa/notice!initNoticeList.action",function(data){
		 if(data.noticeList!=null){
			...
			for(var i = 0;i<noteList.length;i++){
				...
				li = "<li><a style='text-decoration: none;' href='javascript:void(0)' onclick='openNotice("+id+")' ><font color='blue'>"+name+"</font></a>&nbsp;<span class='cor_999'>"+time+"</span>";
				li = li+"</li>";
				noteLi+=li;
			}
			$("#list").append(noteLi);
...
function openNotice(id){
	window.open('<%=urlPath%>parkmanager.oa/notice!viewById.action?id='+id);
}
