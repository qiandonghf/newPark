...
<script type="text/javascript"> 
	...
	function uploadSuccess(file, data, response){
		...
		$.post("<%=basePath%>contactAtt!save.action",postData,function(callbackData){
			...
			setTimeout(function(){
				location.href="<%=basePath%>contact!view.action?type="+$("#type").val()+"&id="+$("#id").val()+"&tabId=2";
			},2000);
		});
	}
	...
	function viewDetails(type,id){
		if('<%=ContactTypeEnum.INVESTMENTCONTACT%>' == type){
			window.open('<%=BaseAction.rootLocation %>/parkmanager.pb/investmentContact!view.action?id='+id,'招商项目审批联系单','height=500,width=800,toolbar=no,menubar=no,scrollbars=yes,resizable=yes, location=no,status=no');
		}else if('<%=ContactTypeEnum.RENTOUTCONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.RENTOUTCONTACT.getTitle()%>','/parkmanager.pb/rentOutContact!view.action?id='+id,800,638);
		}else if('<%=ContactTypeEnum.BUSINESSCENTERCONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.BUSINESSCENTERCONTACT.getTitle()%>','/parkmanager.pb/businessCenterContact!view.action?id='+id,800,638);
		}else if('<%=ContactTypeEnum.TENEMENTCENTERCONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.TENEMENTCENTERCONTACT.getTitle()%>','/parkmanager.pb/tenementCenterContact!view.action?id='+id,800,638);
		}else if('<%=ContactTypeEnum.FINANCECONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.FINANCECONTACT.getTitle()%>','/parkmanager.pb/financeContact!view.action?id='+id,800,638);
		}else if('<%=ContactTypeEnum.CARPORTOUTCONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.CARPORTOUTCONTACT.getTitle()%>','/parkmanager.pb/carportOutContact!view.action?id='+id,800,638);
		}else if('<%=ContactTypeEnum.CUSTOMERSERVICECONTACT%>' == type){
			fbStart('<%=ContactTypeEnum.CUSTOMERSERVICECONTACT.getTitle()%>','/parkmanager.pb/customerServiceContact!view.action?id='+id,800,638);
		}
	}
