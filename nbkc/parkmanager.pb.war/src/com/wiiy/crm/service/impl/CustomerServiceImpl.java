...
public class CustomerServiceImpl implements CustomerService{
	...
	public Result<Customer> save(Customer customer,CustomerInfo customerInfo,IncubationInfo incubationInfo,String labelIds,String[] incubationRouteIds,String[] incubationRouteTimes,String[] customerQualificationIds,String[] custimerQualificationTimes,String[] enterpriseTypeIds) {
		...
		try {
			...
			customer.setMessageSend(EntityStatus.NORMAL);
	...
	@Override
	public void deleteCategoryByIncubation(CustomerCategory customerCategory) {
		Session session = null;
		Transaction tr = null;
		session = customerDao.openSession();
		tr = session.beginTransaction();
		List<CustomerLabel> customerLabels = customerLabelDao.getListByFilter(new Filter(CustomerLabel.class).eq("categoryId", customerCategory.getId()));
		if(customerLabels.size()>0){
			for (CustomerLabel customerLabel : customerLabels) {
				session.createQuery("delete CustomerLabelRef where labelId="+customerLabel.getId()).executeUpdate();
			}
				session.createQuery("delete CustomerLabel where categoryId="+customerCategory.getId()).executeUpdate();
		}
				session.createQuery("delete CustomerCategory where id="+customerCategory.getId()).executeUpdate();
		tr.commit();
		session.close();
	}
	...
	private CardDto populate(Customer customer, String groupName) {
		...
		card.setCompany(customer.getShortName());
