...
public class ContractServiceImpl implements ContractService{
	...
	public Result<String> generateCode(String contractModel) {
		...
		try {
			incubatorName = PbActivator.getAppParamService().loadIncubatorName();
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			count = Integer.parseInt(oldCode.replace(incubatorName+CalendarUtil.now().get(Calendar.YEAR), ""));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//			contract.setName(contract.getCustomerName()+DateUtil.format(new Date(),"yyyyMMddHHmm"));
	...
	public Result saveNetContract1(Contract contract, String[] netIds, String[] prices,
			...
			String[] planFees,String[] realFees, String[] playPayDates, String[] planStartDates,
		...
		try {
			...
			if(netIds!=null){
				for (int i = 0; i < netIds.length; i++) {
					...
					net.setPriceUnit(NetworkPriceUnitEnum.valueOf(priceUnits[i]));
					...
					setcreatemodify(net);
					session.save(net);
					map.put(net.getFacilityId(), net.getId());
					//tempSubject = net;
				}
			}
			...
			for (int i = 0; i < planFees.length; i++) {
				...
				plan.setContractId(contract.getId());
				plan.setPlanFee(Double.valueOf(planFees[i]));
				plan.setRealFee(Double.valueOf(realFees[i]));
				...
				plan.setStatus(BillPlanStatusEnum.UNCHECK);
				/*if(netIds!=null && netIds.length==1){
					plan.setSubjectId(tempSubject.getId());
					plan.setFacilityId(tempSubject.getFacilityId());
				}*/
				plan.setFacilityId(Long.valueOf(planFacilityIds[i]));
				plan.setSubjectId(map.get(plan.getFacilityId()));
				plan.setMemo(memos[i]);
				setcreatemodify(plan);
				session.save(plan);
			}
