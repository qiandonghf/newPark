...
public class BillPlanFacilityAction extends JqGridBaseAction<BillPlanFacility>{
	...
	public String addNetworkPlanByIds(){
		String[] facilityIds = ids.split(",");
		Long[] newIds = new Long[facilityIds.length];
		for(int i=0;i<facilityIds.length;i++){
			newIds[i] = Long.parseLong(facilityIds[i]);
		}
		facilityList = facilityService.getListByFilter(new Filter(Facility.class).in("id", newIds)).getValue();
		return "addNetworkPlanByIds";
	}
