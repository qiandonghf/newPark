...
public class CustomerAction extends JqGridBaseAction<Customer>{
	...
	public String editCustomerBasicInfo(){
		if(id==null){
			customer = customerService.getSessionUserCustomer().getValue();
			if(customer==null) return "editCustomerBasicInfo";
		} else {
	...
	public String createCategoryByIncubation(){//按照孵化状态创建分类
		CustomerCategory customerCategoryTest  = customerCategoryService.getBeanByFilter(new Filter(CustomerCategory.class).eq("name", "孵化状态分组")).getValue();
		if(customerCategoryTest!=null){
			customerService.deleteCategoryByIncubation(customerCategoryTest);
		}
		CustomerCategory customerCategory = new CustomerCategory();
		customerCategory.setName("孵化状态分组");
		customerCategory.setOwnerEnum(OwnerEnum.PUBLIC);
		customerCategory.setOwnerId(PbActivator.getSessionUser().getId());
		...
		if(result.getValue()!=null){
			for(int i=0;i<incubationRouteList.size();i++){
				CustomerLabel customerLabel = new CustomerLabel();
				customerLabel.setCategoryId(customerCategory.getId());
				customerLabel.setOwnerEnum(OwnerEnum.PUBLIC);
				customerLabel.setOwnerId(PbActivator.getSessionUser().getId());
				customerLabel.setName(incubationRouteList.get(i).getDataName());
				customerLabelService.save(customerLabel);
				List<IncubationInfo> incubationInfos = incubationInfoService.getListByFilter(new Filter(IncubationInfo.class).eq("statusName",customerLabel.getName())).getValue();
				for (IncubationInfo incubationInfo : incubationInfos) {
					CustomerLabelRef customerLabelRef = new CustomerLabelRef();
					customerLabelRef.setCustomerId(incubationInfo.getCustomer().getId());
					customerLabelRef.setLabelId(customerLabel.getId());
					customerLabelRefService.save(customerLabelRef);
				}
			}
		}
		return JSON;
	...
	public String loadCategory(){
		...
		//filter.ne("incubationInfoStatus.routeId", "crm.002508").ne("incubationInfoStatus.routeId", "crm.002505").ne("incubationInfo.incubateConfigId","crm.002604");
		TreeDto rjyDto = new TreeDto();
		rjyDto.setId(-3l);
		rjyDto.setText("软件园"+"<input type='hidden' value='rjy|crm.002604'>");
		dtoList.add(rjyDto);
		
		rjyDto = new TreeDto();
		rjyDto.setId(-3l);
		rjyDto.setText("注册地变更"+"<input type='hidden' value='zcdbg|crm.002505'>");
		dtoList.add(rjyDto);
		
		rjyDto = new TreeDto();
		rjyDto.setId(-3l);
		rjyDto.setText("消亡"+"<input type='hidden' value='xw|crm.002508'>");
		dtoList.add(rjyDto);
	...
	public String loadIncubatorRote(){
		...
		for (DataDict dataDict : list) {
			if(dataDict.getDataValue().equals("注册地变更") || dataDict.getDataValue().equals("消亡")){
				continue;
			}
			TreeDto dto = new TreeDto();
			dto.setId(-2L);
	...
	public String export(){
		...
		if(("xw").equals(typeName)){
			filter.eq("incubationInfo.statusName", "消亡").ne("incubationInfo.statusName", "注册地变更").ne("incubationInfo.incubateConfigId","crm.002604");
		}else if(("zcdbg").equals(typeName)){
			filter.ne("incubationInfo.statusName", "消亡").eq("incubationInfo.statusName", "注册地变更").ne("incubationInfo.incubateConfigId","crm.002604");
		}else if(("rjy").equals(typeName)){
			filter.eq("incubationInfo.incubateConfigId", dvalue);
		}else{
		...
		if(routeId!=null && !routeId.equals("") && !routeId.equals("undefined")){
		...
		if(filters.indexOf("incubationInfoStatusRoute.")>-1){
			filter.createAlias("incubationInfo.status", "incubationInfoStatus");
			filter.createAlias("incubationInfoStatus.route", "incubationInfoStatusRoute");
		}
		page=0;//不要分页
		filter.orderBy("code",Filter.ASC);
		refresh(filter);
	...
	public String save(){
		if(customer.getCustomerTypeId().equals("")){
		...
		if(customer.getId()!=null){
			...
			id = ((Customer)result.getValue()).getId();
		}
		return JSON;
	}
	...
	public String list(){
		...
		/*if(sidx!=null && sidx.length()>0 && sord!=null && sord.length()>0){
			if(sidx.equals("customerType")){
				filter.orderBy(sidx, Filter.DESC);
			}
		}*/
		filter.createAlias("customerInfo", "customerInfo");
		filter.createAlias("incubationInfo", "incubationInfo");
		if(("xw").equals(typeName)){
			...
			filter.or(Filter.And(Filter.Ne("incubationInfo.statusName","消亡"), Filter.Ne("incubationInfo.statusName", "注册地变更")), Filter.IsNull("incubationInfo.statusName")).ne("incubationInfo.incubateConfigId","crm.002604");
			//filter.ne("incubationInfo.statusName","消亡").ne("incubationInfo.statusName", "注册地变更").ne("incubationInfo.incubateConfigId","crm.002604");
		}
		if(routeId!=null && !routeId.equals("") && !routeId.equals("undefined")){
			filter.createAlias("incubationInfo.status", "incubationInfoStatus");
			filter.eq("incubationInfoStatus.routeId", routeId);
	...
	public void setCustomerLabelRefService(
			CustomerLabelRefService customerLabelRefService) {
	...
	public void setIncubationInfoService(
			IncubationInfoService incubationInfoService) {
