			        ...
			        success: function(data){
			        	...
			        	if(data.result.success){
			        		setTimeout(function(){
			        		 	/* alert(getOpener());
			        			alert(parent);
			        			var input = parent.document.getElementById("myInput");
			        			alert(input);
			        			 parent.myReload(); 
			        			 getOpener().refresh(); */
			        			parent.fb.end();
				...
				<tr>
					...
					<td class="layerright" style="width:250px;">
						<table border="0" cellspacing="0" cellpadding="0" width="250">
							<tr>
								<td width=""><input id="customerId" value="${result.value.customerId }" name="customerService.customerId" type="hidden" /><input id="customerName" value="${result.value.customer.name }" type="text" class="inputauto" readonly="readonly" /></td>
					...
					<td class="layerright">
						<dd:select key="crm.0024" name="customerService.typeId" checked="result.value.typeId"/>
            		</td>
				</tr>
				<tr>
			 	...
			 	<tr>
			 		...
			 		<td width="30%" class="layerright">
			 			<enum:select type="com.wiiy.crm.preferences.enums.CustomerServiceStatusEnum" name="customerService.status" checked="result.value.status"/>
					</td>
				</tr>
				<tr>
					<td class="layertdleft100">服务结果：</td>
			 		<td class="layerright">
			 			<enum:select type="com.wiiy.crm.preferences.enums.CustomerServiceResultEnum" name="customerService.result" checked="result.value.result"/>
				 	</td>
				</tr>
			 	<tr>
			 		<td class="layertdleft100"><span class="psred">*</span>情况说明：</td>
				 	<td colspan="3" class="layerright"><textarea name="customerService.description" class="textareaauto"  style="height:50px;">${result.value.description }</textarea></td>
			 	</tr>
			 	<tr>
			 		<td class="layertdleft100">企业意见及建议：</td>
				 	<td colspan="3" class="layerright"><textarea name="customerService.suggest" class="textareaauto"  style="height:50px;">${result.value.suggest }</textarea></td>
			 	</tr>
		 	</table>
	 	</div>
