...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function initForm(){
			$("#form1").validate({
				rules: {
					"financeContact.customer":"required",
					"financeContact.roomId":"required",
					"financeContact.content":"required"
				},
				messages: {
					"financeContact.customer":"请选择企业",
					"financeContact.roomId":"请选择房间",
					"financeContact.content":"请填写联系内容"
				},
					...
					$(form).ajaxSubmit({
				        ...
				        success: function(data){
				        	...
				        	if(data.result.success){
				        		setTimeout(function(){		        			
				        			getOpener().reloadList('FINANCECONTACT',data.id);
				        			fb.end();
				        		},2000);
				        	}
				        } 
...
<body>
<form action="<%=basePath %>financeContact!update.action" method="post" name="form1" id="form1">
<!--basediv-->
<div class="basediv">
<input id="id" name="financeContact.id" value="${result.value.id }" type="hidden" />
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td class="layerright"><table width="60%" border="0" cellspacing="0" cellpadding="0">
       						 <tbody><tr>
							          <td><input name="financeContact.type" value="<%=ContactTypeEnum.FINANCECONTACT %>" type="hidden"/>
							          <input id="customerId" name="financeContact.customerId" type="hidden" value="${result.value.customerId }" /><input id="customer" name="financeContact.customer" value="${result.value.customer }" class="inputauto" onclick="selectCustomer()" readonly="readonly"/></td>
						...
						<tr>
							...
							<td class="layerright">
								<table width="60%" border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td>
										<input type="hidden" name="financeContact.roomId" id="roomId" value="${result.value.roomId }"  />
						...
						<tr>
							...
							<td colspan="3" class="layerright" style="padding-top:2px;"> <textarea name="financeContact.content"  class="textareaauto"  rows="4">${result.value.content }</textarea></td>
						</tr>
					</table>
				</div>
