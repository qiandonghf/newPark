...
<form action="<%=basePath %>contect!save.action" method="post" name="form1" id="form1">
	...
	<div class="apptab" id="tableid">
		<ul>
		<%int flag=-1;
		if(PbActivator.getHttpSessionService().isInResourceMap("pb_contectWay_add")){
			flag++;%>
		
			<li class="apptabli<%if(flag==0){ %>over<%} %>" onclick="tabSwitch('apptabli','apptabliover','tabswitch',<%=flag%>)">联系方式</li>
			<%} %>
			<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_extendMessage_add")){flag++; %>
			<li class="apptabli<%if(flag==0){ %>over<%} %>" onclick="tabSwitch('apptabli','apptabliover','tabswitch',<%=flag%>)">扩展信息</li>
			<%} %>
			<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contectOther_add")){flag++; %>
	...
	if(PbActivator.getHttpSessionService().isInResourceMap("pb_contectWay_add")){
		...
		<%} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_extendMessage_add")){
			flag2++;%>
		<div class="basediv tabswitch" style="margin-top:0px; <%if(flag2!=0) {%>display:none;<%} %>" id="textname">
			<div class="divlays" style="margin:0px;">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<%} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contectOther_add")){
			flag2++;%>
		<div class="basediv tabswitch" style="margin-top:0px;<%if(flag2!=0) {%>display:none;<%} %>" id="textname">
			<div class="divlays" style="margin:0px;">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
