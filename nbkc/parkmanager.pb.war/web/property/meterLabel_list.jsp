...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#materLabelDiv').css('height',getTabContentHeight()-58);
	});
	...
	function openRight(id){
		$("#pm_msglist").attr("src","<%=basePath%>meterLabel!view.action?id="+id);
	}
	...
	function jumpPage(page){
		var url = "<%=basePath%>meterLabel!list.action";
		url += "?page="+page;
		if($("#type").val()){
			url += "&type="+$("#type").val();
		}
...
<div class="emailtop">
			...
			<div class="leftemail">
				<ul>
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_meterLabel_add")){ %>
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建分户表抄表','<%=basePath%>web/property/meterLabel_add.jsp',400,360);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
				<%} %>
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="reloadPage();"><span><img src="core/common/images/refresh3.png" /></span>刷新</li>
				</ul>
			</div>
			<!--//leftemail-->
		</div>
	  ...
	  <div class="write_list" style="border-right:1px solid #ddd; width:355px;" id="resizable">
          ...
          <form id="form2" name="form2" method="post" action="">
            <table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td width="65">报表类型： </td>
                <td width="100">
                	<enum:select type="com.wiiy.pb.preferences.enums.MeterTypeEnum" id="type"/>
                </td>
        ...
        <div id="materLabelDiv" style="overflow-x: hidden;overflow-y:auto;">
		    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="lsittable">
              ...
              <c:forEach items="${meterLabelList}" var="meterLabel">
