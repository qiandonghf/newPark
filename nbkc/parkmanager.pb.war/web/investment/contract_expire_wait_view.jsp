...
<%@page import="com.wiiy.crm.entity.ContractExpireApproval"%>
...
<div class="apptab" id="tableid">
	...
	<div class="leftemail">
		<ul>
		<%	
						Result<ContractExpireApproval> result = (Result<ContractExpireApproval>)request.getAttribute("result");
						ContractExpireApproval contractExpireApproval = result.getValue();
						User user = PbActivator.getSessionUser(request);						
					%>
					<%  if(contractExpireApproval.getCwbApprovalId() !=null && contractExpireApproval.getCwbApproval().getUserId().longValue()==user.getId().longValue()) {%>
					...
					<%	} %>
					<%  if(contractExpireApproval.getCwzjApprovalId() !=null && contractExpireApproval.getCwzjApproval().getUserId().longValue()==user.getId().longValue()) {%>
						<li onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="fbStart('财务总监审批','<%=BaseAction.rootLocation %>/core/approval!edit.action?id=${result.value.cwzjApprovalId}',500,170);"><span style="top:5px;"><img src="core/common/images/edit.gif" /></span>财务总监</li>
					<%	} %>
					<%	if(contractExpireApproval.getGcbApprovalId()!=null && contractExpireApproval.getGcb().getUserId().longValue()==user.getId().longValue()) { %>
					...
					<%	} %>
					<%	if(contractExpireApproval.getZjlApprovalId()!=null && contractExpireApproval.getZjlApproval().getUserId().longValue()==user.getId().longValue()) { %>
						<li onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="fbStart('总经理审批','<%=BaseAction.rootLocation %>/core/approval!edit.action?id=${result.value.zjlApprovalId}',500,170);"><span style="top:5px;"><img src="core/common/images/edit.gif" /></span>总经理</li>
					<%	} %>
					<%	if(contractExpireApproval.getKhApprovalId()!=null && contractExpireApproval.getKh().getUserId().longValue()==user.getId().longValue()) { %>
						<li onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="fbStart('客户确认','<%=BaseAction.rootLocation %>/core/approval!edit.action?id=${result.value.khApprovalId}',500,170);"><span style="top:5px;"><img src="core/common/images/edit.gif" /></span>客户</li>
					<%	} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractExpireAtt_save")){ %>
...
<div class="apptable" style=" margin:2px;">
  ...
  <div class="table_footer" style="margin-top: 15px">本联系单填写完毕后，招商部、工程部各留存一份</div>
			...
			<c:forEach items="${attList}" var="att">	
					...
					<div class="downlist">
						...
						<ul>
							...
							<li><%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractExpireAtt_download")) {%><a href="javascript:void" onclick="downLoad('${att.newName}','${att.name}')">下载</a><%}%><%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractExpireAtt_edit")) {%><a href="javascript:void(0)" onclick="fbSearch('重命名','<%=basePath %>ContractExpireApprovalAtt!edit.action?id=${att.id}',350,75);">重命名</a><%}%><%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractExpireAtt_delete")) {%><a href="javascript:void(0)" onclick="deleteFile('${att.id}')">删除</a><%}%></li>
						</ul>
					</div>
					</c:forEach>
		</div>
