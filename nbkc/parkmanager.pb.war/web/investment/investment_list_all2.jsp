				...
				var setUpMenu = <% flag = false;
					...
					data: [[
					 ...
					 if(pb_project_allComplete){
						 if(pb_project_newStatus||pb_project_newStatus){
					 %>
					 ,<%}%>{
						text: "落户",
					 ...
					 if(pb_project_allSleep){
						 if(pb_project_newStatus||pb_project_newStatus || pb_project_allComplete){
					 %>
					 ,<%}%>{
						text: "睡眠",
					 ...
					 if(pb_project_allWakeup){
						 if(pb_project_newStatus||pb_project_newStatus || pb_project_allComplete || pb_project_allSleep){
					 %>
					 ,<%}%>{
						text: "唤醒",
					 ...
					 if(pb_project_allDisagree){
						 if(pb_project_newStatus||pb_project_newStatus || pb_project_allComplete || pb_project_allSleep || pb_project_allWakeup){
							 %>
							 ,<%}%>{
						text: "否决项目",
						...
						<div class="write_list" style="border-right:1px solid #ddd; width:355px;" id="resizable">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" id="lsittable">
								...
								<%
									...
									boolean myInvestment = false;
									if(PbActivator.getSessionUser().getId().equals(investment.getCreatorId())){
										myInvestment = true;
									}
								%>
								<tr onmouseout="this.style.background='#fff';keepHighlight(this)" onmouseover="this.style.background='#f4f4f4';"  class="investmenttr${investment.investmentStatus }<%if(!myInvestment){ %>_other<%} %>" style="cursor: pointer;" onclick="openRight(${investment.id});highlight(this);">
									<%
									...
									%>
									<td class="centertd" style="padding-left: 5px;" ><c:if test="${investment.priority ne null}"><img src="core/common/images/gth<%=priorityColor %>.png" width="4" height="10" /></c:if></td>
									<td class="lefttd" style="text-align: center;">${investment.name}<input type="hidden" value="${investment.id}" /><input type="hidden" value="${investment.regInfo.organizationNumber}" /></td>
									<td class="centertd" style="padding-left: 5px;"><fmt:formatDate value="${investment.modifyTime}" pattern="yyyy-MM-dd"/></td>
								</tr>
								</c:forEach>
							</table>
							</div>
