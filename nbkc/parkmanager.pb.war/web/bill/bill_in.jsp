...
<link rel="stylesheet" type="text/css"
	href="core/common/style/layerdiv.css" />
<link rel="stylesheet" type="text/css"
	href="core/common/calendar/calendar.css" />


<script type="text/javascript" src="core/common/calendar/calendar.js"></script>
<script type="text/javascript" src="core/common/calendar/calendar-zh.js"></script>
<script type="text/javascript"
	src="core/common/calendar/calendar-setup.js"></script>
...
<script type="text/javascript">
	...
	function initList(){
		...
		if(couId  == null || couId == ''){
			path = '<%=basePath%>bill!loadBill.action?type=out' ;
			    ...
			    path = "<%=basePath%>bill!inBill.action?customerId=${param.id}" ;
		}
		var height = 290;
		var width = 685;
		$("#list").jqGrid({
    ...
    function selectPaymentByID(){
    	...
    	if(isIdsNull){
    		...
    		if(ids=='')  return showTip("请选择催缴记录");	
    		fbStart('缴费通知单',"<%=basePath%>bill!inInformListById.action?ids="+ids,800,600);  		
    	}
	} 
    
    function loadCustomerBycoustomerIds(){
		var coustomerIds = $("#customerId").val();
		$("#list").setGridParam({url:'<%=basePath%>bill!inBill.action?customerId='+coustomerIds}).trigger('reloadGrid');
	}
</script>
</head>

<body>
	<search:choose dataType="com.wiiy.crm.preferences.enums.BillInOutEnum"
		field="inOut" op="eq">
		<input type="hidden" class="data" value="IN" />
	</search:choose>
	<search:choose dataType="com.wiiy.crm.preferences.enums.BillStatusEnum"
		field="status" op="eq">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><search:choose dataType="long" field="customerId"
										op="eq">
										<input type="hidden" id="customerId" class="data"
											value="${param.id }" />
									</search:choose> <search:choose dataType="string" field="customer.name" op="cn">
										<input onkeyup="$('#customerId').val('')" readonly="readonly"
											id="customerName" name="endTime" type="text"
											value="<%=customerName%>" class="data inputauto" />
									</search:choose></td>
								<td width="20" align="center"><img
									src="core/common/images/outdiv.gif" width="20" height="22"
									onclick="fbStart('选择企业','<%=BaseAction.rootLocation%>/parkmanager.pb/customer!select.action',520,400);"
									style="cursor: pointer;" /></td>
							</tr>
						</table>
					</td>
					<td colspan="2" class="layerright"><input name="Submit3"
						type="submit" class="search_cx" value="" onclick="loadCustomerBycoustomerIds()" /></td>
				</tr>
			</table>
		</div>
	...
	<div class="basediv">
		<div class="divlays" style="margin: 0px;">
				...
				<div class="leftemail">
					<ul>
						<li onmouseover="this.className='libg'"
							onmouseout="this.className=''" onclick="selectPaymentByID()"><span><img
								src="core/common/images/bill_03.gif" /></span>缴费通知单</li>
						<li onmouseover="this.className='libg'"
							onmouseout="this.className=''"
							onclick="fbStart('添加账单','<%=basePath%>web/bill/bill_add.jsp',500,370);"><span><img
								src="core/common/images/billadd.gif" /></span>补收</li>
						<li onmouseover="this.className='libg'"
							onmouseout="this.className=''" onclick="reloadList()"><span><img
								src="core/common/images/refresh2.gif"></span>刷新</li>
					</ul>
				</div>
			</div>
			<div class="pm_msglist" style="border-top: 0px; margin: 0px;">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					style="margin-top: 1px; background: #f2f2f2">
					<tr>
						<td class="layertdleft100">收款时间：</td>
						<td width="200" class="layerright">
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td><input readonly="readonly"
										value="<%=DateUtil.format(new Date())%>" type="text"
										class="inputauto" id="payTime"
										onclick="showCalendar('payTime')" /></td>
									<td width="25" align="center"><img
										src="core/common/images/timeico.gif" width="20" height="22"
										onclick="showCalendar('payTime')" /></td>
								</tr>
							</table>
						</td>
	...
	<div class="buttondiv">
		<a href="javascript:void(0)" title="" class="btn_bg"
			onclick="income()"><span><img
				src="core/common/images/bill_01.gif" />收款</span></a> <a
			href="javascript:void(0)" title="" class="btn_bg"
			onclick="parent.fb.end();"><span><img
				src="core/common/images/closebtnicon.gif" />取消</span></a>
	</div>
</body>
</html>
