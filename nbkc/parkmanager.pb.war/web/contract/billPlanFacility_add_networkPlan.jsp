...
<input name="billPlanFacility.contractId" type="hidden" value="${id}"/>
<input name="billPlanFacility.customerId" type="hidden" value="${customerId}"/>
<div class="basediv">
	<div class="divlays" style="margin:0px;">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<td class="layerright">
					<select name="billPlanFacility.subjectId" onchange="$('#facilityId').val($('#facilityId'+this.value).val());">
						...
						<c:forEach items="${subjectNetworkList}" var="subjectNetwork">
							<option value="${subjectNetwork.id}">${subjectNetwork.facility.name}</option>
						</c:forEach>
					</select>
					<c:forEach items="${subjectNetworkList}" var="subjectNetwork">
						<input value="${subjectNetwork.facilityId}" type="hidden" id="facilityId${subjectNetwork.id}"/>
						<input value="${subjectNetwork.facility.price}元(${subjectNetwork.facility.checkType.title})" type="hidden" id="price${subjectNetwork.id}"/>
					</c:forEach>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="layerright"><textarea name="billPlanFacility.memo" class="textareaauto" style="height:50px;"></textarea></td>
			</tr>
		</table>
	</div>
