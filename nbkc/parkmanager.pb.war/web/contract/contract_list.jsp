	...
	function initList(){
		...
		$("#list").jqGrid({
			...
			colModel: [
				...
				{label:'合同编号', name:'contractNo', align:"center",width:120}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_view")){ %>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
				<%}%>
			if($(this).getCell(id,"state.title")!='已关闭'){
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractReview_add")){ %>
					content += "<img src=\"core/common/images/application_form_add.png\" width=\"14\" height=\"14\" title=\"发起合同审核会签\" onclick=\"review('"+id+"');\"  /> "; 
				<%}%>
				if($(this).getCell(id,"state.title")=='已执行'){
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contractExpireApproval_add")){ %>
					content += "<img src=\"core/common/images/application_add.png\" width=\"14\" height=\"14\" title=\"发起到期合同审批会签\" onclick=\"expire('"+id+"');\"  /> "; 
				<%}%>
				}
				if($(this).getCell(id,"state.title")!='已执行'){
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_edit")){ %>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
				<%}%>
				}
			}
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_delete")){ %>
...
<div class="emailtop" style="position:relative;z-index:99999;">
	<div class="leftemail">
		<ul>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_add")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="$('#meterlayer').toggle();" style="position:relative; z-index:99999;"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建<em class="em"><img src="core/common/images/arrow.png" /></em>					
				<div class="meterlayer" id="meterlayer" style="display:none; position:absolute; left:5px;" >
					<dl>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_addCustomer")){ %>
					...
					<%} %>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_addFactory")){ %>
					...
					<%} %>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_addOffice")){ %>
					...
					<%} %>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.ZFQY.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_4.jsp?item=<%=ContractItemEnum.ZFQY.name() %>',750,483);"><%=ContractItemEnum.ZFQY.getTitle() %></a></dt>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.FZFQY.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_4.jsp?item=<%=ContractItemEnum.FZFQY.name() %>',750,483);"><%=ContractItemEnum.FZFQY.getTitle() %></a></dt>
						...
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.ZCQLXSQY.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_4.jsp?item=<%=ContractItemEnum.ZCQLXSQY.name() %>',750,483);"><%=ContractItemEnum.ZCQLXSQY.getTitle() %></a></dt>
						<dt><a href="javascript:void(0)" onclick="fbStart('<%=ContractItemEnum.ZCQZFQY.getTitle() %>','<%=basePath %>web/contract/contract_add_customer_rent_4.jsp?item=<%=ContractItemEnum.ZCQZFQY.name() %>',750,483);"><%=ContractItemEnum.ZCQZFQY.getTitle() %></a></dt>
					</dl>
				</div>
			</li>
			<%} %>
			...
			<%} %>
			<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_contract_export")){ %>
...
<div class="searchdiv">
	<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td width="90"><search:choose dataType="com.wiiy.crm.preferences.enums.ContractItemEnum" field="item" op="eq"><enum:select styleClass="data" name="contract.type" type="com.wiiy.crm.preferences.enums.ContractItemEnum"/></search:choose></td>
			<td width="45" align="right">状态：</td>
			<td width="100"><search:choose dataType="com.wiiy.crm.preferences.enums.ContractStatusEnum" field="state" op="eq"><enum:select styleClass="data" name="contract.status" type="com.wiiy.crm.preferences.enums.ContractStatusEnum"/></search:choose></td>
