...
function initList(list){
	...
	$("#"+list).jqGrid({
		...
		colModel: [
			...
			{label:'性质',width:60, name:'kind.dataValue',index:'kind',align:"center"},
			{label:'所在楼宇',width:80, name:'building.name',align:"center"}, 
			{label:'楼层',width:80, name:'floor.name',align:"center"}, 
			{label:'建筑面积',width:80, name:'buildingArea',formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: " ", decimalPlaces: 2, defaulValue: 0},align:"center"}, 
...
<div class="emailtop">
	<div class="leftemail">
		...
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_room_add")){%>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建房间','<%=basePath %>room!addRoom.action',700,427);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
		<%} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_room_delete")){%>
		...
		<%} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_room_search")){%>	
