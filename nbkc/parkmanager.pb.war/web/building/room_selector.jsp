...
<script type="text/javascript">
	...
	function loadRoomByBuildingId(buildingId){
		$("#list").setGridParam({url:'<%=basePath%>room!loadRoomByBuilding.action?id='+buildingId+'&page=1'}).trigger('reloadGrid');
	}
	function doSearch(){
	...
	function initList(){
		$("#list").jqGrid({
	    	url:'<%=basePath%>room!loadRoomByBuilding.action?roomIds='+$("#roomIds").val()+'&customerId='+$("#customerId").val(),
			datatype: 'json',
			mtype: 'POST',
			...
			colModel: [
				...
				{label:'楼宇', name:'building.name', index:'buildingName', align:"center",hidden:true},
