...
<script>
	//园区菜单
	var parkMenu = [[
	<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_building_add")){ %>                
	{
	    text: "新建楼宇",
		classname: "smarty_menu_ico0",
	    func: function() {
			fbStart('新建楼宇信息','<%=basePath%>building!add.action?id='+$(this.parentNode).attr('node-id'),820,472);
	    }
	}
	<%}%>
	],[
	   <%boolean flag=false;
	   if(PbActivator.getHttpSessionService().isInResourceMap("pb_park_edit")){flag=true; %>
	   {
	    text: "重命名",
    ...
    <%}%>
	<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_park_delete")){
			if(flag){%>,<%}%>
	...
	var buildingMenu = [[
     <%flag=false;
     if(PbActivator.getHttpSessionService().isInResourceMap("pb_building_edit")){flag=true;%>
	                     {
	    text: "编辑楼宇",
		classname: "smarty_menu_ico1",
	    func: function() {
...
<%}%>
<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_building_delete")){
	if(flag){%>,<%}%>
	                     {
	    text: "删除楼宇",
		classname: "smarty_menu_ico2",
			...
			$("#tt").tree({
				...
				onClick: function(node){
					...
					if($(this).tree("isLeaf",node.target)){
						$("#msglist").attr("src","<%=basePath%>building!view.action?id="+id);
					}
				},
			...
			<tr>
	        	<td width="182" valign="top">
					<div class="agency" id="resizable">
						...
						<div class="agencybtn">
							<ul>
							<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_park_add")){ %>
				...
				<td width="100%" valign="top">
					<div>
						<iframe src="parkmanager.pb/web/building/park_index.jsp" frameborder="0" id="msglist" width="100%"></iframe>
					</div>
				</td>
			</tr>
		</table>
