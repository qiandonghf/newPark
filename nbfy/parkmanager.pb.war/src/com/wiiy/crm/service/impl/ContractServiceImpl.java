...
public class ContractServiceImpl implements ContractService{
	...
	public Result<String> generateCode(String contractModel) {
		...
		Contract contract = contractDao.getBeanByFilter(new Filter(Contract.class).ge("createTime", CalendarUtil.getEarliest(Calendar.YEAR)).like("contractNo", code).orderBy("id", Filter.DESC).maxResults(1));
		int count = contractDao.getRowCount(new Filter(Contract.class).ge("createTime", CalendarUtil.getEarliest(Calendar.YEAR)).like("contractNo", code));
		if(contract!=null){
			try {
