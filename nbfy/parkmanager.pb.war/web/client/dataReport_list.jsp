...
<script>
	...
	var groupMenu = [[
	     <%if(PbActivator.getHttpSessionService().isInResourceMap("pb_dataReport_add")){%>             
	 {
	    ...
	    func: function() {
			fbStart('新建报表','<%=basePath%>dataReport!add.action?id='+$(this).find('input').val(),500,180);
	    }
	}
	 <%}%>
	 ],[
	    ...
	    func: function() {
			fbStart('编辑分组','<%=basePath%>dataReportGroup!edit.action?id='+$(this).find('input').val(),300,72);
	    }
	}
	<%}%>
	...
	var dataReportMenu = [[
	 ...
	 if(PbActivator.getHttpSessionService().isInResourceMap("pb_dataReport_edit")){
		...
		classname: "smarty_menu_ico1",
	    func: function() {
			fbStart('编辑报表','<%=basePath%>dataReport!edit.action?id='+$(this).find('input').val(),500,210);
	    }
	}
		<%}%>
					...
					<div class="agency" id="resizable">
						...
						<div class="agencybtn">
							...
							<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_dataReportGroup_add")){ %>
								<li><a href="javascript:void(0)" onclick="fbStart('新建分组','<%=basePath %>web/client/dataReportGroup_add.jsp',300,72);"><span class="icoadd"></span>新建分组</a></li>
							<%} %>
							<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_dataReport_add")){ %>
								<li><a href="javascript:void(0)" onclick="fbStart('新建报表','<%=basePath%>dataReport!add.action',500,210);"><span class="icoadd"></span>新建报表</a></li>
							<%} %>
							</ul>
						</div>
