...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>complaint!list.action',
			colModel: [
					{label:'状态', width:'100' ,name:'status.title', align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_handle")){%>
					content += "<img src=\"core/common/images/cl.png\" width=\"14\" height=\"14\" title=\"处理\" onclick=\"handle('"+id+"');\"  /> "; 
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_delete")){%>
	...
	function viewById(id){
		fbStart('查看投诉单','<%=basePath %>complaint!view.action?id='+id,600,329);
	}
	
	function editById(id){
		fbStart('编辑投诉单','<%=basePath %>complaint!edit.action?id='+id,600,239);
	}
...
<div class="emailtop">
			<div class="leftemail">
				<ul>
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_add")){%>
					<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建投诉单','<%=basePath %>web/property/complaint_add.jsp',600,239);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
				<%} %>
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_complaint_delete")){%>
