...
function initList(list){
	var url = "";
	if($("#vacantRoom").val()!=null){
		url = "<%=basePath%>room!idleRoomList.action?id="+$("#vacantRoom").val();
	}else{
		url = "<%=basePath%>room!idleRoomList.action";
	}
	...
	$("#"+list).jqGrid({
		...
		colModel: [
			...
			{label:'所在楼宇',width:100, name:'building.name',align:"center"}, 
...
<form action="<%=basePath%>room!export.action" id="exportForm" method="post">
	...
	<input type="hidden" name="exportName" value="置空单元列表"/>
</form>
<div class="emailtop">
<input type="hidden" id="vacantRoom" value="${param.id}"/>
	<div class="leftemail">
		<ul>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_room_delete")){%>
