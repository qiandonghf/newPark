...
<script type="text/javascript">
	...
	function initList(){
		$("#list").jqGrid({
	    	url:'<%=basePath%>investment!selectList.action',
			datatype: 'json',
			mtype: 'POST',
			...
			colModel: [
				...
				{label:'注册资本', name:'regCapital', index:'regCapital', align:"center",hidden:true},
				{label:'企业人数', name:'staff', index:'staff', align:"center",hidden:true},
				{label:'经营范围', name:'businessScope', index:'businessScope', align:"center",hidden:true},
				{label:'实际办公地址', name:'address', index:'address', align:"center",hidden:true},
				{label:'法定代表人', name:'regInfo.legalPerson', index:'regInfo.legalPerson', align:"center",hidden:true},
				{label:'用房面积', name:'officeArea', index:'officeArea', align:"center",hidden:true},
				{label:'计划总投资', name:'investCapital', index:'investCapital', align:"center",hidden:true},
				{label:'预计年产值', name:'outputValue', index:'outputValue', align:"center",hidden:true}
			],
			height: 240,
			...
			if($("#type").val()==1){
				...
				$(ids).each(function (index, id) {
					if ($("#list").find('#'+id+' input').prop('checked')) {
						...
						var legalPerson = $("#list").jqGrid("getCell", id,'regInfo.legalPerson');
						parent.setSelectedInvestment(rowData,legalPerson);
						return;
					}
				});
			}
