...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
			...
			colModel: [
				{label:'楼号',width:80, name:'building.name', align:"center"}, 
				{label:'分类', name:'type.dataValue', width:80 , align:"center"}, 
			    ...
			    {label:'企业', name:'customerName', align:"center",width:180},
			    {label:'合同到期日期', name:'endDate', align:"center", width:120,formatter:'date',formatoptions:{srcformat: 'Y-m-d', newformat: 'Y-m-d'}},
			    {label:'备注', name:'memo', align:"center",width:150},
			],
			shrinkToFit: false,
			height: height,
			width: width,
				...
				<div class="titlebg">出租明细表</div>
					...
					<div class="msglist" style="display: block;" id="textname"
						...
						<area shape="rect" coords="0,0,53,22" href="javascript:void(0)"
							onclick="doSearch()" />
						<%-- <area shape="rect" coords="55,0,74,22" href="javascript:void(0)" onclick="fbSearch('高级搜索','<%=basePath %>web/contract/contract_search.jsp',550,400);" /> --%>
					</map>
				</td>
			</tr>
		</table>
