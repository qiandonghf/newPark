...
<script type="text/javascript">
  ...
  function exportData(){
		...
		location.href ="<%=basePath%>bill!exportBillWzubByProperty.action?startTime="+startTime+"&endTime="+endTime;
  }
                          ...
                          <form id="form2" name="form2" method="post" action="">
                   	    ...
                   	    <div class="table_layout" id="chartdiv11" style="padding-top: 140px;">
							<script language="JavaScript">					
								var chart1 = new FusionCharts("<%=BaseAction.rootLocation %>/core/common/FusionChartsFree/Charts/FCF_Pie2D.swf", "ChartId", "325", "250");		   			
								/* var xml = "<graph palette='2' caption='物业应收未收图' shownames='1' showvalues='0' baseFontSize='12' decimalPrecision='2' limitsDecimalPrecision='0' divLineDecimalPrecision='0' bgColor='' showAlternateHGridColor='1' alternateHGridColor='' divlinecolor=''>"; */
								var xml = "<graph decimalPrecision='0' caption='物业应收未收图' showNames='1' showValues='0' baseFontSize='12'>";
								xml += "<%
									 ...
									 for(int j =0; j<groupList.get(m).getGroups().size();j++){
											 ...
											 if(groupList.get(m).getGroups().get(j).getName().equals(typeName)){
												 for(int k = 0;k<groupList.get(m).getGroups().get(j).getList().size();k++){
													 dvalue1 += groupList.get(m).getGroups().get(j).getList().get(k).getdValue();
													 dvalue2 += groupList.get(m).getGroups().get(j).getList().get(k).getdValue2();
												 }
											 }
										 }
									}
								}
								BigDecimal bd1 = new BigDecimal(dvalue1);//应收 
								BigDecimal bd2 = new BigDecimal(dvalue2);//未收
								double dvalue3 = bd1.subtract(bd2).doubleValue();//实收
								out.print("<set name='实收' value='"+dvalue3+"' color='#FFA500' />");
								out.print("<set name='未收' value='"+dvalue2+"' color='#F08080' />");
								%>";
								xml += "</graph>";
