...
<script type="text/javascript">
	...
	function toSubmit(){
		...
		if(confirm('确认催缴帐单？')){
			var postData = {urged:'',email:'',sms:''};
			if($("#urged").attr("checked")=='checked') postData.urged = "NO";
			if($("#email").attr("checked")=='checked') postData.email = "yes";
			if($("#sms").attr("checked")=='checked') postData.sms = "yes";
			$.post("<%=basePath %>bill!inInform.action",postData,function(data){
	...
	function inform(urged){
		location.href="<%=basePath%>bill!inInformList.action?urged="+urged;
	}
</script>
</head>
...
系统检测到现有${customerTotal}个企业共计${billTotal}条帐单需缴费处理，其中${urgedTotal}条帐单已催缴，选择催缴范围与催缴方式实现催缴操作。如需打印催缴通知单，则进入&nbsp;<a href="javascript:void(0)" title=""  onclick="fbStart('收款','<%=basePath%>bill!in.action',700,370);">收款结算</a>&nbsp;页面。</div>
<div style="padding:5px 5px; text-align:center;">
	<label><input id="urged" type="radio" class="vetM" name="radio" <c:if test="${urged eq 'NO'}">checked="checked"</c:if> onclick="inform('NO')"/><span class="vetM">未催缴帐单催缴</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<label><input type="radio" class="vetM" name="radio" <c:if test="${urged ne 'NO'}">checked="checked"</c:if> onclick="inform('YES')"/><span class="vetM">所有帐单催缴</span></label>
</div>
...
<div class="buttondiv">
  ...
  <a href="javascript:void(0)" title="" class="btn_bg" onclick="fbStart('缴费通知单打印','<%=basePath%>bill!printPayMentIn.action?ids=${ids }',730,400);"><span><img src="core/common/images/print_btn.gif" />打印</span></a>
  <a href="javascript:void(0)" title="" class="btn_bg" onclick="toSubmit()"><span><img src="core/common/images/bill_07.gif" />催缴</span></a>
