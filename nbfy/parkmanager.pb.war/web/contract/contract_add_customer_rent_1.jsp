...
String fees = "";
for(RentBillPlanFeeEnum fee : RentBillPlanFeeEnum.values()){
...
<title>无标题文档</title>
	...
	<script type="text/javascript">
		...
		function generateCode(){
			...
			<%-- if(confirm("若生产Z类合同，请点击‘确定’；若生成S类合同，请点击‘取消’")){
				$.post("<%=basePath%>contract!generateCode.action?contractModel=Z",function(data){
					if(data.result.success){
						$("#contractNo").val(data.result.value);
					}
				});
			}else{
				$.post("<%=basePath%>contract!generateCode.action?contractModel=S",function(data){
					if(data.result.success){
						$("#contractNo").val(data.result.value);
					}
				});
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" >
		<tr>
			<td class="layertdleft100"><span class="psred">*</span>合同编号：</td>
      		...
      		<% 
      			String propertyName = "";
      			if(PbActivator.getAppConfig().getConfig("propertyName")!=null){
      				propertyName = PbActivator.getAppConfig().getConfig("propertyName").getParameter("name");
      			}
      			String address = "";
      			if(PbActivator.getAppConfig().getConfig("address")!=null){
      				address = PbActivator.getAppConfig().getConfig("address").getParameter("name");
      			}
      			
      		%>
      		<td class="layerright" width="25%"><input name="contract.propertyName" type="text" class="inputauto" style="width:99%;" value="<%=propertyName %>" /></td>
    	</tr>
    	<tr>
	      	<td class="layertdleft100">所租房屋产权证号：</td>
	      	<td class="layerright"><input name="contract.propertyNo" type="text" class="inputauto" style="width:99%;" value="" /></td>
	      	<td class="layertdleft100">座落位置：</td>
	      	<td class="layerright" width="33%"><input name="contract.address" type="text" class="inputauto" style="width:99%;" value="<%=address %>" /></td>
      	</tr>
      	<tr>
	      	<td class="layertdleft100"><span class="psred">*</span>房屋类型：</td>
	      	<td class="layerright" >
	      		<dd:select name="contract.roomTypeId" key="pb.0006"/>
	      	</td>
      	...
      	<tr>
     		...
     		<td class="layerright"><table width="143" border="0" cellspacing="0" cellpadding="0">
		        <tr>
		          <td><input id="managerId" name="contract.managerId" type="hidden" value="<%=PbActivator.getSessionUser(request).getId() %>"/><input id="managerName" name="managerName" class="inputauto" onclick="selectUser()" readonly="readonly" value="<%=PbActivator.getSessionUser(request).getRealName() %>"/></td>
