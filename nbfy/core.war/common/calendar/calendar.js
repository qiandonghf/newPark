...
//	(this.weekNumbers) && ++title_length;
	...
	this._nav_py = hh("<img id=objyprev src=\"core/common/images/yprev.gif\">", 1, -2);
...
//	this._nav_py = hh("&#x00ab;", 1, -2);
	...
	//	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny = hh("<img id=objynext src=\"core/common/images/ynext.gif\">", 1, 2);
	this._nav_ny.width = 25;
	this._nav_ny.ttip = Calendar._TT["NEXT_YEAR"];

//	this._nav_pm = hh("&#x2039;", 1, -1);
	this._nav_pm = hh("<img id=objmprev src=\"core/common/images/mprev.gif\">", 1, -1);
...
//	this._nav_nm = hh("&#x203a;", 1, 1);
	this._nav_nm = hh("<img id=objmnext src=\"core/common/images/mnext.gif\">", 1, 1);
...
//	this._nav_now.ttip = Calendar._TT["GO_TODAY"];
	...
	if (this.isPopup) {
		...
		var td_close = hh("<img id=objclose src=\"core/common/images/close.gif\">", 1, 200);
...
Calendar.prototype.showAtElement = function (el, opts) {
	 ...
	 */
	try{
		if(typeof(eval("getCalendarScrollTop")) == "function"){
			p.y -= getCalendarScrollTop();
		}
	}catch(e){
	}
	/**
	 * 调整日历坐标如果下边界超出窗口则显示在输入框上方
	 */
	if(p.y+w>document.documentElement.clientHeight){
		p.y = p.y - el.offsetHeight - h;
		if(p.y<0) p.y=0;
	}
	this.showAt(p.x, p.y);
};
