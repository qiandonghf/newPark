...
<script type="text/javascript">
		...
		$.post("<%=BaseAction.rootLocation %>/parkmanager.edi/report!list.action",function(data){
			if(data.result!=null && data.result.success){
				...
				for(var i = 0;i<reportList.length;i++){
					...
					li = "<li>"+yearNo+"年&nbsp;";
					if(expired=='3'){
						li += "<a href='javascript:void(0)' onclick='alert(\"报表已过期,请联系管理员\");'>";
					}else{
		...
		$.post("<%=BaseAction.rootLocation %>/parkmanager.pb/customer!workBenchCustomerList.action",function(data){
			...
			if(customerList!=null){
				...
				for(var i=0;i<customerList.length;i++){
					...
					var a = "<a href=\"javascript:void(0)\" onclick=\"fbStart('查看企业档案','<%=BaseAction.rootLocation %>/parkmanager.pb/customer!view.action?id="+customer.id+"',870,450);\">"+customer.name+"</a>&nbsp;";
					li += title+a+"</li>";
					customerLi+=li;
				}
	...
	function openCustomer(obj){
		var id = $(obj).attr("id");
		fbStart('查看企业','<%=BaseAction.rootLocation %>/parkmanager.pb/customer!view.action?id='+id+'& desktop =true',870,450);
	}
