...
public class DataReportToCustomerAction extends JqGridBaseAction<DataReportToCustomer>{
	...
	public String reportProfit(){
		result = dataReportToCustomerService.reportProfit(id,status,propertyIds,propertyValues);
		return JSON;
	}
	/*public String reportDebt(){
		result = dataReportToCustomerService.reportDebt(id,status,propertyIds,propertyValues);
		return JSON;
	}*/
	...
	public String view(){
		...
		for (DataReportValue dataReportValue : dataReportValueList) {
			dataReportValueMap.put(dataReportValue.getProperty().getPropertyId(), dataReportValue);
		}
		if(dataTemplate.getId()==1){
			return "viewProfit";
		}else if(dataTemplate.getId()==2){
			return "viewDebt";
		}
		return "error";
	}
	...
	public String edit(){
		...
		view();
		DataReportToCustomer reportToCustomer = (DataReportToCustomer)result.getValue();
		if(reportToCustomer.getReport().getTemplate().getId()==1){
			return "editProfit";
		}else if(reportToCustomer.getReport().getTemplate().getId()==2){
			return "editDebt";
		}
		return "error";
	}
