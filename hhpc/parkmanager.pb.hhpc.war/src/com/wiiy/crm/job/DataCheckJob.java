				...
				for (Room room : roomList) {
					if(room.getKind().getDataValue().equals("出租")){
						if(executeContractMap.get(room.getId())!=null){
							...
							output.write("<tr><td><span style='color:red;'>"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态不正常,没有相关标的在执行"+"</span></td></tr>");
							System.err.println("****************"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态不正常");
						}
					}else{
						if(executeContractMap.get(room.getId())!=null){
							output.write("<tr><td><span style='color:red;'>"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态不正常，合同【"+executeContractMap.get(room.getId()).getContractNo()+"】中有相关标的在执行"+"</span></td></tr>");
							System.err.println("****************"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态不正常，合同【"+executeContractMap.get(room.getId()).getContractNo()+"】中有相关标的在执行");
						}else{
							output.write("<tr><td><span>"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态正常"+"</span></td></tr>");
							System.out.println("================"+room.getBuilding().getName()+"【"+room.getName()+"】的房间状态正常");
						}
