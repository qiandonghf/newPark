...
public class DataReportServiceImpl implements DataReportService{
	...
	@Override
	public Result<Map<Long,DataReportPropertyDto>> count(DataReport report) {
		Map<Long,DataReportPropertyDto> map = new HashMap<Long, DataReportPropertyDto>();
		String propertyID = "select property_id,id FROM "+table(DataReportProperty.class)+
				" where report_id = "+report.getId();
		List<Object> propertyList = dataReportDao.getObjectListBySql(propertyID);
		Map<Long, Long> propertyIdMap = new HashMap<Long, Long>();
		for (Object object : propertyList) {
			Object[] os = (Object[])object;
			Long propertyId = BigInteger.class.cast(os[0]).longValue();
			Long reportId = BigInteger.class.cast(os[1]).longValue();
			propertyIdMap.put(reportId,propertyId);
		}
		
		String sumSql = "SELECT v.property_id,SUM(v.double_val)" +
				" FROM "+table(DataReportValue.class)+" v" +
				" LEFT JOIN "+table(DataReportToCustomer.class)+" c ON v.report_customer_id = c.id" +
				" WHERE c.report_id = " + report.getId() +
		...
		for (Object object : sumList) {
			...
			map.put(propertyIdMap.get(propertyId), dto);
		}
	...
	public Result<List<DataReportPropertyDto>> count(Long id) {
		...
		for (Object object : sumList) {
			...
			if(dto.getProperty().getDataType()!=null){
