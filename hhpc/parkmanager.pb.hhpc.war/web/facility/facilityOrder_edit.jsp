...
<script type="text/javascript">
	$(function(){
		...
		initForm();
		initUploadify();
		loadContract(${result.value.customerId});
	});
		...
		$.post("<%=basePath %>contract!loadContractByCustomerId.action?id="+id,function(data){
			if(data.result.success){
				...
				for(var i = 0; i < list.length; i++){
				 ...
				 var contractPath = $("option:contains('${result.value.contractPath}')").index();
				 if(contractPath>list.length){
					 $("#contractId").get(0).selectedIndex = 0;
				 }else{
					 $("#contractId").get(0).selectedIndex = contractPath;
				 }
			}
		});
	}
...
<body>
<form action="<%=basePath %>facilityOrder!update.action" method="post" name="form1" id="form1">
	<input type="hidden" value="${result.value.id}" name="facilityOrder.id"/>
	<input type="hidden" value="${result.value.facilityId}" name="facilityOrder.facilityId"/>
	<input type="hidden" value="${result.value.contractPath}" id="contractPath"/>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><input id="customerId" name="facilityOrder.customerId" type="hidden" class="inputauto" value="${result.value.customerId}"/><input value="${result.value.customer.name}" readonly="readonly" id="customerName" name="customerName" type="text" class="inputauto" onclick="fbStart('选择企业','<%=BaseAction.rootLocation %>/parkmanager.pb/customer!select.action',520,390);"/></td>
				...
				<tr>
					...
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="150">
									<input readonly="readonly" id="startTime" name="startTime" type="text" class="inputauto" onclick="showCalendar('startTime')" value="<fmt:formatDate value="${result.value.startTime}" pattern="yyyy-MM-dd"/>"/>
								...
								<td>
									<%
										Result<FacilityOrder> result = (Result<FacilityOrder>)request.getAttribute("result"); 
				...
				<tr>
					...
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td width="150">
									<input readonly="readonly" id="endTime" name="endTime" type="text" class="inputauto" onclick="showCalendar('endTime')" value="<fmt:formatDate value="${result.value.endTime}" pattern="yyyy-MM-dd"/>"/>
									<input id="facilityOrderEndTime" name="facilityOrder.endTime" type="hidden" value="test"/>
								</td>
								<td width="20"><img id="endTimeIcon" style="position: relative;left:-1px;" src="core/common/images/timeico.gif" width="20" height="22" onclick="showCalendar('endTime')"/></td>
								<td>
									<%
				...
				<tr>
					...
					<td class="layerright"><textarea name="facilityOrder.memo" rows="6" class="textareaauto">${result.value.memo}</textarea></td>
				</tr>
			</table>
		</div>
