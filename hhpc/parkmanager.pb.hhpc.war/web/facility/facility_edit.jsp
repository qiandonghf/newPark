...
<script type="text/javascript">
	...
	function checkBuilding(){
		loadBuildingByParkId($("#parkId").val());
	}
	...
	function deletePhoto(img){
		...
		$(img).parent().remove();
		$.post("<%=BaseAction.rootLocation %>/core/resources/"+$(img).parent().prev().find("input").val()+"-d");
		checkPhotosDisplay();
	}
		...
		$.post("<%=basePath%>building!loadBuildingByParkId.action?id="+parkId,function(data){
			if(data.result.success){
				...
				for(var i = 0; i < data.result.value.length; i++){
					...
					if(building.id=='${result.value.buildingId}'){
						select.append($("<option></option>",{value:building.id,selected:"selected"}).append(building.name));
					} else {
...
<body>
<form action="<%=basePath %>facility!update.action" method="post" name="form1" id="form1">
	<input type="hidden" value="${result.value.id}" name="facility.id"/>
	<div class="basediv">
		<div class="divlays" style="margin:0px;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="layertdleft100"><span class="psred">*</span>设施名称：</td>
					<td colspan="3" class="layerright"><input name="facility.name" type="text" class="inputauto" value="${result.value.name}"/></td>
				</tr>
				<tr>
					<td class="layertdleft100"><span class="psred">*</span>园区：</td>
					<td class="layerright">
						<select onchange="loadBuildingByParkId(this.value)" id="parkId" name="facility.parkId">
							<option value="">----请选择----</option>
							<c:forEach items="${parkList}" var="park">
								<option value="${park.id}" <c:if test="${park.id==result.value.parkId}">selected="selected"</c:if>>${park.name}</option>
							</c:forEach>
						</select>
					</td>
				...
				<tr>
					...
					<td class="layerright"><enum:select name="facility.status" type="com.wiiy.pb.preferences.enums.FacilityStatusEnum" checked="result.value.status"/></td>
					<td class="layertdleft100"><span class="psred">*</span>是否收费：</td>
					<td width="182" class="layerright"><enum:radio styleClass="bcharge" name="facility.bcharge" type="com.wiiy.commons.preferences.enums.BooleanEnum" checked="result.value.bcharge"/></td>
				</tr>
				<tr>
					<td class="layertdleft100">结算类型：</td>
					<td class="layerright"><enum:select id="checkType" name="facility.checkType" type="com.wiiy.pb.preferences.enums.CheckOutTypeEnum" checked="result.value.checkType"/></td>
					<td class="layertdleft100">计费类型：</td>
					<td width="182" class="layerright"><enum:select id="billingMethod" name="facility.billingMethod" type="com.wiiy.pb.preferences.enums.BillingMethodEnum" checked="result.value.billingMethod"/></td>
				</tr>
				<tr>
					<td class="layertdleft100">单价：</td>
					<td class="layerright"><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td><input id="price" name="facility.price" type="text" class="inputauto" value="${result.value.price}"/></td><td width="25" align="center">元</td></tr></table></td>
					<td class="layertdleft100">是否独享资源：</td>
					<td width="182" class="layerright"><enum:radio name="facility.bshare" type="com.wiiy.commons.preferences.enums.BooleanEnum" checked="result.value.bshare"/></td>
				</tr>
				<tr>
				...
				<tr id="photosTr" <c:if test="${result.value.photos eq ''}">style="display: none;"</c:if>>
						...
						<div style="width:300px; overflow-x:auto; overflow-y:hidden; height:80px; margin-bottom:2px;">
							<table border="0" cellspacing="0" cellpadding="10">
								...
								<c:forTokens items="${result.value.photos}" delims="," var="photo">
									<td><input type="hidden" value="${photo}" class="photo"/><img src="core/resources/${photo}" width='50' height='50' class='productimg'/></td>
									<td valign="bottom"><img src="core/common/images/locking.jpg" onclick="deletePhoto(this)"/></td>
								</c:forTokens>
								</tr>
							</table>
						</div>
				...
				<tr>
					...
					<td colspan="3" class="layerright"><textarea name="facility.summary" rows="6" class="textareaauto">${result.value.summary}</textarea></td>
				</tr>
			</table>
		</div>
