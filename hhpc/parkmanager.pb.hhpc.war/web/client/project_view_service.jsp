...
<script type="text/javascript">
	...
	function initProject(){
		$("#productList").jqGrid({
			...
			colModel: [
			    {label:'项目/产品名称', name:'name',align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplyView")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('项目信息','product','"+id+"',500,352);\"  /> ";
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplyEdit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('项目信息','product','"+id+"',500,503);\"  /> ";
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplyDel")){%>
		...
		$("#projectApplyList").jqGrid({
			...
			colModel: [
				...
				{label:'项目/产品名称', name:'product.name',align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplySBView")){%>
					...
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplySBEdit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('项目申报情况','projectApply','"+id+"',500,519);\"  /> ";
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplySBDel")){%>
          ...
          <tr>
            ...
            <td valign="top" style="background:#eaeced ">
				...
				<div class="emailtop">
					<div class="leftemail">
						<ul>
						<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplyAdd")){ %>
				...
				<div class="basediv tabswitch" style="margin:0px;display:none;" name="textname" id="textname">
					<div class="emailtop">
						<div class="leftemail">
							<ul>
							<%if(PbActivator.getHttpSessionService().isInResourceMap("ps_serviceCenter_projectApplySBAdd")){ %>
