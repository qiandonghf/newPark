...
<script type="text/javascript">
	...
	function initForm(){
		$("#form1").validate({
			rules: {
				"customerService.customerName":"required",
			...
			messages: {
				"customerService.customerName":"请选择企业",
				...
				$(form).ajaxSubmit({
			        ...
			        success: function(data){
			        	...
			        	if(data.result.success){
			        		setTimeout(function(){
			        			getOpener().location.href = '<%=basePath%>customerService!list.action?id='+data.result.value.id;
			        			parent.fb.end();
			        		},2000);
			        	}
			        } 
...
<body>
<form action="<%=basePath %>customerService!save.action" method="post" name="form1" id="form1">
	<div class="basediv">
				...
				<tr>
					...
					<td class="layerright" style="width:250px;">
						<table border="0" cellspacing="0" cellpadding="0" width="250">
							<tr>
								<td width=""><input id="customerId" value="" name="customerService.customerId" type="hidden" /><input id="customerName" name="customerService.customerName" value="" type="text" class="inputauto" /></td>
					...
					<td class="layerright">
						<dd:select key="crm.0024" name="customerService.typeId"/>
            		</td>
				</tr>
				<tr>
					<td class="layertdleft100"><span class="psred">*</span>联系人：</td>
			        <td class="layerright">
			           <input name="customerService.contect" type="text" class="inputauto" style="width:98%;" />
			        </td>
					<td class="layertdleft100"><span class="psred">*</span>受理日期：</td>
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
					          <td><input name="customerService.startDate" readonly="readonly" type="text" class="inputauto" id="startDate"  value="<fmt:formatDate value='<%=new Date()%>' pattern='yyyy-MM-dd'/>"    onclick="return showCalendar('startDate');" /></td>
				 ...
				 <tr>
				 	...
				 	<td class="layerright"><input name="customerService.phone" type="text" class="inputauto" style="width:98%;" /></td>
				 	<td class="layertdleft100"><span class="psred">*</span>受理人：</td>
				 	<td class="layerright"><%=PbActivator.getSessionUser(request).getRealName()%>
			          	<input type="hidden" name="customerService.userId" value="<%=PbActivator.getSessionUser(request).getId()%>"/>
			        </td>
			 	</tr>
			 	<tr>
			 		<td class="layertdleft100"><span class="psred">*</span>情况说明：</td>
				 	<td colspan="3" class="layerright"><textarea name="customerService.description"  class="inputauto"  style="height:50px;resize:none;padding-bottom:2px;"></textarea></td>
			 	</tr>
		 	</table>
	 	</div>
