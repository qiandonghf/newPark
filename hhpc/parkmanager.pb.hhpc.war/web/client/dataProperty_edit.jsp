...
<script type="text/javascript">
	...
	function initParent(){
		$('#parentId').combotree({
		    ...
		    onChange:function(){
		    	$("#pId").val($("#parentId").combotree("getValue"));
		    }
		});
		if('${result.value.parentId}'.length==0) {
			$('#parentId').combotree("setValue","----请选择----");
		} else {
			$('#parentId').combotree("setValue","${result.value.parentProperty.name}");
		}
	}
	
	function expandChildren(node){
		if(node.state=="closed") {
			$("#parentId").tree("expand",node.target);
			setTimeout(function(){
	...
	function expandAll(){
		var roots = $("#parentId").tree("getRoots");
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td class="layertdleft100">数据项分组：</td>
       <td class="layerright">
       <input onchange="changParent()" id="parentId" style="width:180px;"/>
     </td>
