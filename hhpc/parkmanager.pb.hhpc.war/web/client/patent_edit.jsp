	...
	function initForm(){
		$("#form1").validate({
			...
			submitHandler: function(form){
				if(notNull("applyTime","专利申请日期") && notNull("buyTime","授权公告日期")){
					if($("#buyTime").val()<$("#applyTime").val()){
						showTip("授权公告日期不能小于专利申请日期",5000);
						return;
					}
				}
				if(notNull("expireTime","专利失效日期") && notNull("buyTime","授权公告日期")){
					if($("#expireTime").val()<$("#buyTime").val()){
						showTip("专利失效日期不能小于授权公告日期",5000);
						return;
					}
				}
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td class="layertdleft100">专利类型：</td>
       <td class="layerright">
      	<dd:select id="type" name="patent.typeId" key="crm.0010" checked="result.value.typeId"/>
     </td>
      <td class="layertdleft100">获得方式：</td>
      <td class="layerright">
      	<dd:select id="source" name="patent.sourceId" key="crm.0012" checked="result.value.sourceId"/>
      	<enum:radio name="patent.sourceFrom" type="com.wiiy.crm.preferences.enums.PatentSourceEnum" checked="result.value.sourceFrom" />
     </td>
    </tr>
    <tr>
     ...
     <tr>
      ...
      <td class="layerright">
      	<dd:select id="state" name="patent.stateId" key="crm.0011" checked="result.value.stateId"/>
   	  </td>
