...
<script type="text/javascript">
	...
	function showView(name,id){
		fbStart(name,'<%=basePath %>customer!view.action?id='+id,890,460);
	}
	...
	function initList(){
		...
		$("#list").jqGrid({
			...
			colModel: [
			    ...
			    {label:'中心联络员', name:'hostName', align:"center", hidden:true},
	...
	function viewById(id){
		fbStart($($("#list").getRowData(id).name).text(),'<%=basePath %>customer!view.action?id='+id,870,463);
	}
	function viewInvestmentById(investmentById){
		fbStart('项目信息','<%=basePath %>investment!view.action?id='+investmentById,700,364);
	}
	function editById(id){
		fbStart('编辑企业档案','<%=basePath %>customer!edit.action?id='+id,700,551);
	}
	function reloadList(){
...
<div class="emailtop">
	<div class="leftemail">
		...
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_customer_add")){ %>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建企业档案','<%=basePath %>customer!add.action',700,541);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
		<%} %>
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_customer_delete")){ %>
							...
							<tr>
								...
								<td width="105">
									<table width="100%" border="0" cellspacing="2" cellpadding="0">
										<tr>
											<td width="180">
												<search:input inputClass="inputauto" id="name" dataType="string" field="name" op="cn"/>
											</td>
						  				</tr>
									</table>
								</td>
								...
								<td width="105">
									<search:choose id="name" dataType="com.wiiy.crm.preferences.enums.CustomerTypeEnum" field="type" op="eq" >
