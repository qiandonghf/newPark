...
<script type="text/javascript">
	$(function(){
		...
		initForm();
		initUploadify();
		<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_hatchMessage_add")){ %>
	...
	function checkApplication(){
		...
		if(apply == 'YES'){
			...
			if(content ==''){
				...
				$("#content").val(CKEDITOR.instances.content.getData());
				$('#pub').val('<%=CustomerApplyState.APPLYING %> ');			
			}
		}
		return true;
		<%-- var content = CKEDITOR.instances.content.document.getBody().getText();
		...
		if(content ==''){
			...
			$.ajax({
				   type: "POST",
				   url: "<%=basePath %>customer!apply.action",
...
<form action="<%=basePath %>customer!save.action" method="post" name="form1" id="form1">
	...
	<input id="pub" name="customer.pub" type="hidden"/>
	<div class="basediv">
		...
		<div class="divlays" style="margin:0px;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td colspan="3" class="layerright"><input id="name" name="customer.name" type="text" class="inputauto" /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="layerright">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td>
									<input id="gardenId" name="customer.gardenApplyId" type="hidden" class="inputauto" />
									<input id="gardenName" readonly="readonly" type="text" class="inputauto" onclick="selectGarden()" />
								</td>
					...
					<td class="layerright">
						<enum:select name="customer.base" type="com.wiiy.crm.preferences.enums.CustomerBaseEnum" />
					</td>
				</tr>
				<tr>
	...
	<div>
		<div class="apptab" id="tableid">
			<ul>
				<%int flag=-1;
				if(PbActivator.getHttpSessionService().isInResourceMap("pb_customerMessage_add")){flag++;%>
				...
				<table id="fixedTable" width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr style="padding-bottom:2px;">
						...
						<td class="layerright">
							<select id="selectApply">
								<option value="NO">暂不申请</option>
								<option value="YES">立即申请</option>
							</select>
						</td>
						<td class="layertdleft100">申请状态：</td>
						<td class="layerright"><%=CustomerApplyState.NOAPPLICATION.getTitle() %></td>
					</tr>
					<tr>
				      ...
				      <tr>
				      <td height="60" width="100"> <input type="file" id="fileUpload" /><input type="hidden" id="photos" name="customer.logo" /> <div id="fileQueue"></div> </td>
				      <td>
					      <table  border="0" cellspacing="0" cellpadding="0">
					      	<tr id="imageList"></tr>
					      </table>
					...
					<tr>
						...
						<td colspan="3" class="layerright" style="padding-bottom:2px;">
							<textarea id="content" name="customer.content"  class="textareaauto" style="height:100px;margin-top:2px;">
