...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	...
	    	colModel: [
				...
				{label:'装机容量', width:'70' ,name:'capacity', align:"center"}, 
				{label:'基本电费', width:'70' ,name:'capacityBill', align:"center"}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					var text = id+"|"+roomId;
					var content = "";
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_waterMeter_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_waterMeter_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
					<%}%>
					<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_waterMeter_delete")){%>
	...
	function viewById(id){
		fbStart('查看电表','<%=basePath %>meter!view.action?id='+id,600,248);
	}
	
	function editById(id){
		fbStart('编辑电表','<%=basePath %>meter!edit.action?id='+id,600,248);
	}
