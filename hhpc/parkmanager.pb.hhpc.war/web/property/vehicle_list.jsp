...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>vehicleManagement!list.action',
			colModel: [
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					var id = ids[i];
					var content = "";
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_vehicle_view")){%>
					content += "<img src=\"core/common/images/viewbtn.gif\" width=\"14\" height=\"14\" title=\"查看\" onclick=\"viewById('"+id+"');\"  /> "; 
				<%}%>
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_vehicle_edit")){%>
					content += "<img src=\"core/common/images/edit.gif\" width=\"14\" height=\"14\" title=\"编辑\" onclick=\"editById('"+id+"');\"  /> "; 
				<%}%>
				<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_vehicle_delete")){%>
	...
	function editById(id){
		fbStart('编辑车辆管理','<%=basePath %>vehicleManagement!edit.action?id='+id,600,165);
	}
...
<body>
	<div class="emailtop">
		<div class="leftemail">
			<ul>
			<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_vehicle_add")){ %>
				<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('新建车辆管理','<%=basePath %>web/property/vehicle_add.jsp',600,165);"><span><img src="core/common/images/emailadd.gif" width="15" height="13" /></span>新建</li>
			<%} %>
			<%if(PbActivator.getHttpSessionService().isInResourceMap("pb_vehicle_delete")){ %>
