...
<script type="text/javascript">
	...
	function save(type){
		if('opinion1'== type){
			...
			if(opinion == ""){
				...
				var id=$("#id").val();
				var url="<%=basePath%>customerServiceContact!approval.action?approvalType=opinion1&id="+id;
	...
	function setSelectedUser(user){
		var id = $("#id").val();
		var url = "<%=basePath%>customerServiceContact!send.action?id="+id+"&approvalType="+approvalType+"&receiveId="+user.id;
...
<input type="hidden" id="customerId" value="${result.value.customerId }"/>
     <tr>
        <td colspan="2" width="10%">企业名称</td>
        <td colspan="7">${result.value.customer.name }</td>
      </tr>
      <tr>
       	<td colspan="2">联系人</td>
        <td colspan="3">${result.value.linkman }</td>
        <td colspan="2">服务类型</td>
        <td colspan="2">${result.value.serviceType.dataValue }</td>
      </tr>
      <tr>
       	<td colspan="2">联系电话</td>
        <td colspan="3">${result.value.telephone }</td>
        <td colspan="2">受理开始日期</td>
        <td colspan="2"><fmt:formatDate value="${result.value.startDate}" pattern="yyyy-MM-dd"/></td>
      </tr>
      <tr>
        <td colspan="2">受理人</td>
        <td colspan="3">${result.value.creator}</td>
        <td colspan="2">受理结束日期</td>
        <td colspan="2"><fmt:formatDate value="${result.value.endDate}" pattern="yyyy-MM-dd"/></td>
      </tr>
      <tr>
        <td colspan="2">服务结果</td>
        <td colspan="3">${result.value.result.title}</td>
        <td colspan="2">状态</td>
        <td colspan="2">${result.value.status.title}</td>
      </tr>
      <tr>
        <td colspan="2">情况说明</td>
        <td colspan="7">${result.value.description}</td>
      </tr>
      <tr>
        <td colspan="2">企业意见及建议</td>
        <td colspan="7">${result.value.suggest}</td>
      </tr>
    </table>
