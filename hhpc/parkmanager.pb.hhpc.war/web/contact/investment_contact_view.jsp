...
<input type="hidden" id="id" value="${result.value.id }"/>
      ...
      <tr>
        ...
        <td>座机</td>
        <td colspan="3">${linkman.telephone}</td>
      </tr>
    </table>
  ...
  <tr>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin:2px 0;">
 	  <tr>
        ...
        <td class="layerright" style="padding-top:4px;" colspan="3"><textarea name="textarea"  class="textareaauto" <c:if test="${result.value.handleOpinionId ne userId || result.value.handleOpinion ne null}">readonly="readonly"</c:if> id="handleTxt" style="height:50px;">${result.value.handleOpinion }</textarea></td>
        <td width="6%"><input type="hidden" id="handleOpinionId" value="${result.value.handleOpinionId }" /><label id="handleLab">
	        ...
	        <%-- <c:if test="${result.value.handleOpinionId eq null && result.value.handleOpinion eq null }">
	        	<input name="Submit" type="button" onclick="send('handleOpinion');" class="allbtn" value="发送" />
	        </c:if> --%>
        </label></td>
      </tr>
      <tr>
        <td class="layertdleft100"><span class="tsy-h80">招商人员意见：</span></td>
        <td class="layerright" style="padding-top:4px;" colspan="3"><textarea name="textarea"  class="textareaauto" <c:if test="${result.value.attractOpinionId ne userId || result.value.attractOpinion ne null}">readonly="readonly"</c:if> id="attractTxt" style="height:50px;">${result.value.attractOpinion }</textarea></td>
        <td width="6%"><input type="hidden" id="attractOpinionId" value="${result.value.attractOpinionId }" /><label id="attractLab">
	       ...
	       <%--  <c:if test="${result.value.attractOpinionId eq null && result.value.attractOpinion eq null }">
	        	<input name="Submit" type="button" onclick="send('attractOpinion');" class="allbtn" value="发送" />
	        </c:if> --%>
        </label></td>
      </tr>
      <tr>
        <td class="layertdleft100"><span class="tsy-h80">投资促进部意见：</span></td>
        <td class="layerright" style="padding-top:4px;" colspan="3"><textarea name="textarea"  class="textareaauto" <c:if test="${result.value.departmentOpinionId ne userId || result.value.departmentOpinion ne null}">readonly="readonly"</c:if> id="departmentTxt" style="height:50px;">${result.value.departmentOpinion }</textarea></td>
      ...
      <tr>
        ...
        <td class="layerright" style="padding-top:4px;" colspan="3"><textarea name="textarea" id="headTxt" <c:if test="${result.value.headOpinionId ne userId || result.value.headOpinion ne null}">readonly="readonly"</c:if> class="textareaauto"  style="height:50px;">${result.value.headOpinion }</textarea></td>
