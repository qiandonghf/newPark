...
<script type="text/javascript">
	...
	function save(type){
		if('opinion1'== type){
			...
			if(opinion == ""){
				...
				var id=$("#id").val();
				var url="<%=basePath%>financeContact!approval.action?approvalType=opinion1&id="+id;
	...
	function setSelectedUser(user){
		var id = $("#id").val();
		var url = "<%=basePath%>financeContact!send.action?id="+id+"&approvalType="+approvalType+"&receiveId="+user.id;
...
<input type="hidden" id="roomId" value="${result.value.roomId }"/>
 ...
 <div class="hqxx" style="padding:5px 0 0;">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin:2px 0;">
      <tr>
        <td class="layertdleft100"><span class="tsy-h80">财务部回馈意见：</span></td>
