  ...
  <div id="mainright2"> 
        ...
        <div class="parklist">
	        <ul>
	          ...
	          <% num ++; %>
	          <li>
	          	<span><fmt:formatDate value="${article.pubTime }" pattern="yyyy-MM-dd" /></span>
	          	<em><img src="images/ico3.gif" width="8" height="5" /></em><c:if test="${empty category.parentId }">『${article.articleType.typeName }』</c:if><a href="view.action?id=${article.id }&pos=${pager.page},${pager.records}.<%=num%>">${article.title }</a>
	          </li>
	          </c:forEach>
	        </ul>
		</div> 
