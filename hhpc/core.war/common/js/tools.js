	...
	function getParam(id){
		if(getOb(id)!=null && getMyValue(id)!=null && getMyValue(id)!='')
			return '&'+id+'='+getMyValue(id);
		return "";
	}
	function getParams(){
		var result = '';
	...
	function checkLength(id,min,max){
		return getMyValue(id).length>=min && getMyValue(id).length<=max;
	}
	//验证字符串是否超过最大长度(如验证用户名最长不能超过50位)
	function checkMaxLength(id,max){
		return getMyValue(id).length<=max;
	}
	//验证字符串是否小于最小长度(如验证密码最短为4位)
	function checkMinLength(id,min){
		return getMyValue(id).length>=min;
	}
	//验证身份证号码(15到18位数字)
	...
	function checkEqual(id,id2){
		return getMyValue(id)==getMyValue(id2);
	}
	function checkOb(id,norm,msg){
		if(getMyValue(id).search(norm) < 0){
			showTip(msg+"格式错误");
			return false;
		}
		return true;
