				...
				$.each(dataMenu, function(i, arr) {
					...
					if ($.isArray(arr)) {
						$.each(arr, function(j, obj) {
							var text = obj.text,classname = obj.classname, htmlMenuLi = "", strTitle = "", rand = Math.random().toString().replace(".", "");
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li" data-hover="true">' + htmlCreateMenu(obj.data) +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"> <span class="'+ classname +'"></span><i class="'+ clKey +'triangle"></i>'+ text +'</a>' + 
									'</li>';
								} else {
									htmlMenuLi = '<li class="'+ clKey +'li">' +
										'<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"><span class="'+ classname +'"></span>'+ text +'</a>' + 
									'</li>';
								}
			...
			if (!jqueryMenu.size()) {
				...
				$(idKey + params.name +" li").each(function() {
					...
					$(this).hover(function() {
						...
						if (isHover) {					
							var childrenBox = $(this).addClass(clHover).children("."+ clKey +"box");
							var divTop = childrenBox.parents(".smart_menu_box").position().top;
							var parentTop = childrenBox.parent().position().top;
							var top = childrenBox.height()+divTop+parentTop;
							if(top>document.documentElement.clientHeight){
								top = document.documentElement.clientHeight-top;
								childrenBox.css("top", top);
							}
							childrenBox.show();
		...
		$(this).each(function() {
			this.oncontextmenu = function(e) {
				...
				if (jqueryMenu) {
					...
					if(jqueryMenu.height()+e.clientY>document.documentElement.clientHeight){
						//top = e.clientY - jqueryMenu.height();
						top = document.documentElement.clientHeight-jqueryMenu.height();
					}
					jqueryMenu.css({
						display: "block",
