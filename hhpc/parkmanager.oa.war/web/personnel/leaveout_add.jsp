...
<body>
<form id="form1" name="form1" method="post" action="<%=basePath %>leaveRegister!save.action">
<!--basediv-->
  ...
  <table width="100%"  class="oatableAdd" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td width="150" class="oatabletdright">
	  	 <input name="leaveRegister.applicant" />
	  </td>
    </tr>
	<tr>
	 <td class="layertdleftblack">申请日期：</td>
      <td class="oatabletdright">
	  	 <input type="hidden" name="leaveRegister.applyTime" value="<%=DateUtil.format(new Date(), "yyyy-MM-dd")%>"/>
				...
				<tr>
					<td>
						<c:forEach items="${enumList}" var="leaveType" begin="0" end="3">
							<input type="radio" name="leaveRegister.leaveType" value="${leaveType}"/>&nbsp;${leaveType.title}
						</c:forEach>
					</td>
				</tr>
				<tr>
    ...
    <tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
			  <td>
				<input id="startTime" name="leaveRegister.startTime" type="hidden" value='<fmt:formatDate value="<%=new Date() %>" pattern="yyyy-MM-dd HH:mm:ss"/>'/>
			  	   从
		  ...
		  <tr>
			 <td>
			 <input id="endTime" name="leaveRegister.endTime" type="hidden" value='<fmt:formatDate value="<%=new Date() %>" pattern="yyyy-MM-dd HH:mm:ss"/>'/>
			 	   至
   ...
   <tr>
    	...
    	<td  class="oatabletdright" style="padding:2px 5px;">
			<label>
			<textarea name="leaveRegister.memo" class="textareaauto" style="width:400px; height:40px;"></textarea>
			</label>		
		</td>
   </tr>
</table>
