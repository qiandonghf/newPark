...
<%@page import="com.wiiy.oa.entity.UserSign"%>
...
<body>
<form id="form1" name="form1" method="post" action="<%=basePath%>userSign!update.action">
<!--basediv-->
<div class="basediv">
	<!--titlebg-->
	<!--//titlebg-->
...
<div class="divlays" style="margin:0px;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td class="layerright">
      	<input type="hidden" name="userSign.id" value="${result.value.id}"/>
      	<table  border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td width="120"><input id="userId" name="userSign.userId" value="${result.value.userId}" type="hidden" /><input id="username" name="realName" value="${result.value.user.realName}" class="inputauto" readonly="readonly" onclick="fbStart('选择登记人','<%=BaseAction.rootLocation %>/core/user!select.action',520,400);"/></td>
				<td width="20" align="center"><img style="cursor:pointer;" onclick="fbStart('选择登记人','<%=BaseAction.rootLocation %>/core/user!select.action',520,400);" src="core/common/images/outdiv.gif" width="20" height="22" /></td>
				<td align="center">&nbsp;</td>
			</tr>
		</table></td>
    </tr>
    <tr>
      ...
      <td class="layerright" width="400">
		...
		<tr>
			<td width="120">
				<input id="signTime" name="signTime" type="text" class="inputauto" readonly="readonly" onclick="return showCalendar('signTime','y-mm-dd');" value="<fmt:formatDate value="${result.value.signTime}" pattern="yyyy-MM-dd"/>"/>
      ...
      <td width="100" class="layerright"><select name="userSign.workClassId">
        	<option value="">--选择班次--</option>
         	<c:forEach items="${workClassList}" var="workClass">
         		<option value="${workClass.id}" <c:if test="${result.value.workClassId == workClass.id}">selected="selected"</c:if>>${workClass.name}</option>      	
        	</c:forEach>              
      </select></td>
    </tr>
    <tr>
      <td class="layertdleft100"><span class="psred">*</span>补签标识：</td>
      <td class="layerright"><enum:radio name="userSign.signType" checked="result.value.signType" type="com.wiiy.oa.preferences.enums.SignTypeEnum"/></td>
    </tr>
    <tr>
      <td class="layertdleft100"><span class="psred">*</span>迟到或早退原因：</td>
      <td><textarea name="userSign.reason" rows="4" class="textareaauto">${result.value.reason}</textarea></td>
    </tr>
  </table>
</div>
<!--//divlay-->
