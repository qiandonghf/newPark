...
<div class="emailtop">
	<div class="leftemail">
		<ul>
			<li style="background-color:#D2E6FF" onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="location.href='<%=basePath %>workDayReport!depCountList.action'"><span><img src="core/common/images/daylycount.gif" /></span>日报</li>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''"  onclick="location.href='<%=basePath %>workReport!depWeekCountList.action'"><span><img src="core/common/images/weeklycount.png" /></span>周报</li>
  ...
  <table border="0" width="665" cellspacing="0" cellpadding="0" class="oacount">
   <tr>
    	...
    	<img src="core/common/images/dataviewleft.png" width="15" height="16" onclick="location.href='<%=basePath %>workDayReport!depCountList.action?yearStr=<fmt:formatDate value="${prevYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/></em>
    	${year}年<em style="position:relative; top:3px; padding-left:5px;">
    	<img src="core/common/images/dataviewright.png" onclick="location.href='<%=basePath %>workDayReport!depCountList.action?yearStr=<fmt:formatDate value="${nextYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/></em>&nbsp;&nbsp;&nbsp;<em style="position:relative; top:3px; padding-right:5px;">
    	<img src="core/common/images/dataviewleft.png" width="15" height="16" onclick="location.href='<%=basePath %>workDayReport!depCountList.action?monthStr=<fmt:formatDate value="${prevMonth}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/></em>
    	${mon}月<em style="position:relative; top:3px; padding-left:5px;">
    	<img src="core/common/images/dataviewright.png" onclick="location.href='<%=basePath %>workDayReport!depCountList.action?monthStr=<fmt:formatDate value="${nextMonth}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/></em></td>
    	</tr>
    <tr>
    ...
    <tr>
      ...
      <td class=""><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
          <tr>
         	 <c:forEach items="${monthDto.dayDtoList}" var="dayDto">
         	 	<c:if test="${dayDto.lateSignInNum==1}">
         	 		 <td class="workcounttd"><img src="core/common/images/rightgreen.png" onclick="javascript:vopen('<%=basePath %>workDayReport!viewDepWork.action?id=${workCountDto.id}&dateStr=<fmt:formatDate value="${dayDto.date}" pattern="yyyy-MM-dd HH:mm:ss"/>','days',800,300);" width="7" height="7" /></td>
         	 	</c:if>
         	 	<c:if test="${dayDto.lateSignInNum==0}">
         	 		 <td class="workcounttd">&nbsp;</td>
         	 	</c:if>
         	 	<c:if test="${dayDto.lateSignInNum==null}">
         	 		 <td class="workcounttd">&nbsp;</td>
         	 	</c:if>
            </c:forEach>
          </tr>
