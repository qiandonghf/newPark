...
<div class="emailtop">
	<div class="leftemail">
		<ul>
			...
			<li style="background-color:#D2E6FF" onmouseover="this.className='libg'" onmouseout="this.className=''" 	onclick="location.href='<%=basePath %>workReport!depMonthCountList.action'"><span><img src="core/common/images/monthcount.png" /></span>月报</li>
  ...
  <tr>
    	<td colspan="13" align="center" class="workcountth"><em style="position:relative; top:3px; padding-right:5px;">
    		<img src="core/common/images/dataviewleft.png" width="15" height="16" onclick="location.href='<%=basePath %>workReport!depMonthCountList.action?myYear=<fmt:formatDate value="${prevYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/>
    			</em>${year}年<em style="position:relative; top:3px; padding-left:5px;">
    		<img src="core/common/images/dataviewright.png" onclick="location.href='<%=basePath %>workReport!depMonthCountList.action?myYear=<fmt:formatDate value="${nextYear}" pattern="yyyy-MM-dd HH:mm:ss"/>'"/>
    		</em>
    	</td>
      </tr>
    <tr>
	 ...
	 <td width="65" class="workcounttd"><table width="100%" border="0" cellspacing="0" cellpadding="0">
		 ...
		 <c:forEach items="${workCountDto.monthDtoList}" var="monthDto">
		 	<c:if test="${monthDto.num==1}">
		 		<td class="workcounttd"><img src="core/common/images/rightgreen.png" width="7" height="7" onclick="javascript:vopen('<%=basePath %>workReport!viewDepMonth.action?myMonth=${monthDto.month}&curYear=${year}&id=${workCountDto.id}','month',800,600);"/></td>
		 	</c:if>
	 		<c:if test="${monthDto.num==0 or monthDto.num==null}">
