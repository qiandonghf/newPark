...
<body>
<form action="<%=basePath %>meetingRoomRent!update.action" method="post" name="form1" id="form1">
<input type="hidden" name="meetingRoomRent.id" value="${result.value.id}"/>
<!--basediv-->
<div class="basediv">
<div class="divlays">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="layerright"><input name="meetingRoomRent.company" type="text" value="${result.value.company}" class="inputauto"/></td>
        <td class="layertdleft100"><span class="psred">*</span>借用事由：</td>
        <td class="layerright"><input name="meetingRoomRent.reason" type="text" value="${result.value.reason}" class="inputauto"/></td> 
      </tr>
      <tr>
        <td class="layertdleft100"><span class="psred">*</span>联系电话：</td>
        <td class="layerright"><input name="meetingRoomRent.phone" type="text" value="${result.value.phone}" class="inputauto"/></td>
	 	<td class="layertdleft100"><span class="psred">*</span>联系人：</td>
        <td class="layerright"><input name="meetingRoomRent.linkman" type="text" value="${result.value.linkman}" class="inputauto"/></td>
      </tr>
      <tr>
	 	<td class="layertdleft100"><span class="psred">*</span>参会人数：</td>
        <td class="layerright"><input name="meetingRoomRent.cnt" type="text" value="${result.value.cnt}" class="inputauto"/></td>
      </tr>
      <tr>
        ...
        <td class="layerright">
			...
			<tr>
				<td><input id="rentTime" name="meetingRoomRent.rentTime"  type="text" class="inputauto" readonly="readonly" onclick="return showCalendar('rentTime','yy-mm-dd');" value="<fmt:formatDate value="${result.value.rentTime}" pattern="yyyy-MM-dd"/>"  /></td>
			...
			<label>
				<select id="sendMinute" >
					<c:forEach begin="0" end="59" var="s" step="15">
						<option value="<c:if test="${s<10}">0</c:if>${s}" <%if(Integer.parseInt(pageContext.getAttribute("s").toString())==minute) out.println("selected=\"selected\"");%>><c:if test="${s<10}">0</c:if>${s}</option>
					</c:forEach>
				</select>
			</label>
			分
      ...
      <tr>
        ...
        <td colspan="3"><span class="layerright">
          <textarea name="meetingRoomRent.meetingRoom" rows="4" class="textareaauto">${result.value.meetingRoom}</textarea>
        </span></td>
      </tr>
      <tr>
        <td class="layertdleft100">办公室意见：</td>
        <td colspan="3"><span class="layerright">
          <textarea name="meetingRoomRent.approval" rows="4" class="textareaauto">${result.value.approval}</textarea>
        </span></td>
      </tr>
    </table>
