...
function initList(){
	...
	$("#list").jqGrid({
    	url:'<%=basePath%>supplyPurchaseApply!listAll.action',
		colModel: [
			{label:'名称', name:'supply.name',width:"105", align:"center"}, 
			{label:'型号', name:'supply.spec',width:90,align:"center"}, 		   
		    {label:'数量', name:'amount',width:"100", align:"center",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: " ", decimalPlaces: 2, defaulValue: 0}},
		    {label:'单位', name:'supply.unit', width:"90",align:"center"},
		    {label:'单价', name:'price', width:"90",align:"center",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: " ", decimalPlaces: 2, defaulValue: 0}},
		    {label:'总价', name:'totalPrice', width:"90",align:"center",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: " ", decimalPlaces: 2, defaulValue: 0}},
		    {label:'日期', name:'applyTime',width:"85", align:"center",formatter:'date',formatoptions:{srcformat:'Y-m-d',newformat:'Y-m-d'}},
		    {label:'采购人', name:'purchaser', width:"90",align:"center"},
		    {label:'备注', name:'comment', width:"100",align:"center"},
...
function viewById(id){
	fbStart('查看','<%=basePath%>supplyPurchaseApply!view.action?id='+id,400,376);
}
function editById(id){
	fbStart('编辑申请单','<%=basePath %>supplyPurchaseApply!edit.action?id='+id,500,376);
}
function deleteById(id){	
	if(confirm("确定要删吗")){
		$.post("<%=basePath%>supplyPurchaseApply!delete.action?id="+id,function(data){
			if(data.result!=null){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
...
function deleteByIds(){
	...
	if(ids!='' && confirm("确定要删吗")){
		$.post("<%=basePath%>supplyPurchaseApply!delete.action?ids="+ids,function(data){
			if(data.result!=null){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
...
<body>
<form action="<%=basePath%>supplyPurchaseApply!export.action" id="exportForm" method="post">
		...
		<div class="leftemail">
			<ul>
				<li onmouseover="this.className='libg'"
					...
					onclick="fbStart('新建办公用品采购申请','<%=basePath %>web/chiefadmin/Office_supplies_purchase_add.jsp',500,376);"><span><img
