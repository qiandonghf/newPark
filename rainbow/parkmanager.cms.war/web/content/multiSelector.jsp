...
<script type="text/javascript">
  ...
  function getUsers(){
	  if(objIds && objIds != 'undefined'&& objIds != 'null'){
		  var ids = objIds.substr(1,objIds.length-2).split(",");
		  for(var i=0;i<ids.length;i++){
			  var obj = $("#"+ids[i]);
			  var child = $("#list").find('#'+ids[i]+' input[type=checkbox]');
			  $(obj).addClass("ui-state-highlight");
			  $(obj).attr("aria-selected","true");
			  var name = $(obj).children().eq(2).attr("title");
			  $(child).attr("checked","checked");
			  selectedUsers[i] = {id:ids[i],name:name};
		  }
		  checkSelectedUsers();
	  }
  }
  function showSelectedUser() {
	  ...
	  for(var i =0 ;i<selectedUsers.length;i++){
		  selectedUserText += selectedUsers[i].name + ",";
		}
  ...
  function initList(){
		$("#list").jqGrid({
	    	url:'<%=BaseAction.rootLocation%>/core/user!listSelf.action',
			datatype: 'json',
 ...
 */					$(this).jqGrid('setRowData',id,{manager:content});
	...
	function submitSelectedUser() {
		getOpener().setWebManager(selectedUsers,objId);
		fb.end();
	}
</script>
</head>
