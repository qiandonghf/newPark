...
<script type="text/javascript">
	...
	function initList(){
		...
		$("#list").jqGrid({
	    	url:'<%=basePath%>article!listByNormal.action?kind=SINGLE&paramId=${param.paramId}',
			colModel: [
				...
				{label:'所属栏目',width:160,name:'articleType.typeName',align:"center"}, 
				{label:'点击量',width:80,name:'hits',align:"center"}, 
				{label:'发布人', hidden:true,width:120,name:'editor', align:"center"},
				{label:'发布', hidden:true,name:'pubed.title',width:40,align:"center"}, 
				{label:'发布时间', name:'pubed.pubTime',align:"center",width:80,hidden:true,formatter:'date',formatoptions:{srcformat:'Y-m-d',newformat:'Y-m-d'}}, 
	...
	function viewById(id){
		fbStart('查看','<%=basePath%>article!view.action?id='+id,800,490);
	}	
	...
	function loadByTypeId(typeId,paramId){
		$("#list").setGridParam({url:'<%=basePath%>article!loadByTypeId.action?typeId='+typeId+'&paramId='+paramId+'&kind=SINGLE'}).trigger('reloadGrid');
	}
	
	function listByNormal(paramId){
		$("#list").setGridParam({url:'<%=basePath%>article!listByNormal.action?kind=SINGLE&paramId='+paramId}).trigger('reloadGrid');
	}
...
<body>
<div class="titlebg">单页文章</div>
<!--emailtop-->
<div class="emailtop">
	<div class="leftemail">
		<ul>
			<%-- <%if(add){%>
			<li onmouseover="this.className='libg'" onmouseout="this.className=''" onclick="fbStart('文章新建','<%=basePath %>web/content/content_addByNormal.jsp',800,490);"><span><img src="core/common/images/emailadd.gif"/></span>添加</li>
			<%} %>
			<%if(delete){%>
			...
			<li>
				<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
			      <tr>
			        ...
			        <td width="120"><label>
			        	<input id="searchContent" name="searchContent" class="data inputauto"/>
			        </label></td>
