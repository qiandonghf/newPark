...
<script type="text/javascript">
  var name = "${param.name}";
  name = decodeURI(name);
  var selectedUser={id:"${param.id}", name:name};
  ...
  function initList(){
	var memo = "${param.memo}";
	var url = "${contextLocation}parkmanager.cms/param!list.action";
	if(memo == 'article'){
		url = "${contextLocation}parkmanager.cms/param!listArticle.action?articleKind=${param.kind}"
	}
	$("#list").jqGrid({
    	url:url,
		datatype: 'json',
		...
		colModel: [
		    ...
		    {label:'网站名称', name:'name', index:'name', align:"center"}
		    ],
		height: 204,
	...
	function checkSelectedUser() {
		...
		$(ids).each(function (index, id) {
			if ($("#list").find('#'+id+' input').prop('checked')) {
				...
				selectedUser = {id : id, name : rowData.name};
			}
		});
		showSelectedUser();
	}  
	...
	function submitSelectedUser() {
		if (selectedUser.id == "") {
			showTip("请先选择一个网站", 1000);
			return;
		}
		getOpener().setSelectedParam(selectedUser);
		fb.end();
	}
</script>
</head>
