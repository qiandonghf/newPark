...
<script type="text/javascript">
	...
	function initList(){
		var height = $(this).height()-79;
		var width = $(this).width()-1;
		$("#list").jqGrid({
	    	url:'<%=basePath%>platform!list.action',
			colModel: [
				{label:'平台名称', name:'name',align:"center",width:160}, 
				{label:'平台标题', name:'title',align:"center",width:160}, 
				{label:'栏目名称', name:'articleType.typeName',align:"center",width:120}, 
				{label:'地址', name:'address',align:"center",width:160}, 
				{label:'电话', name:'phone',align:"center",width:120}, 
			...
			gridComplete: function(){
				...
				for(var i = 0 ; i < ids.length; i++){
					...
					boolean add = CmsActivator.getHttpSessionService().isInResourceMap("contentManagement__platform_add");
					boolean view = CmsActivator.getHttpSessionService().isInResourceMap("contentManagement__platform_view");
					boolean edit = CmsActivator.getHttpSessionService().isInResourceMap("contentManagement__platform_edit");
					boolean delete = CmsActivator.getHttpSessionService().isInResourceMap("contentManagement__platform_delete");
					%>
					<%if(view){%>
	...
	function add(){
		fbStart('新建','<%=basePath %>web/service/platform_add.jsp',800,555);	
	}
	function viewById(id){
		fbStart('查看','<%=basePath%>platform!view.action?id='+id,670,458);
	}	
	
	function editById(id){
		fbStart('编辑','<%=basePath%>platform!edit.action?id='+id,670,568);
	}
	...
	function publishById(id){
		if(confirm("确定要发布吗")){
			$.post("<%=basePath%>platform!publish.action?id="+id,function(data){
	...
	function backById(id){
		if(confirm("确定要撤回吗")){
			$.post("<%=basePath%>platform!back.action?id="+id,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		$("#list").trigger("reloadGrid");
	        	}
	...
	function deleteById(id){
		if(confirm("确定删除意见")){
			$.post("<%=basePath%>platform!delete.action?id="+id,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		reloadList();
	        	}
	...
	function deleteSelected(){
		...
		if(ids==""){
			showTip('请先选择一个平台',2000);	
		}else if(confirm("确定要删除这些平台")){
			$.post("<%=basePath%>platform!delete.action?ids="+ids,function(data){
				showTip(data.result.msg,2000);
	        	if(data.result.success){
	        		reloadList();
	        	}
