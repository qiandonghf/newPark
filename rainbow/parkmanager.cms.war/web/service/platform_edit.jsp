...
<script type="text/javascript" >
	...
	function initForm(){
		$("#form1").validate({
			...
			messages: {
				"platform.name":"请填写平台名称",
				"platform.title":"请填写平台标题",
				"param.name":"请选择所属网站",
	...
	function deleteAtt(old,path,oldName,newName){
		$.ajax({
			type:"POST",
			 url: "<%=basePath %>platform!deleteByFilePath.action",
			data:"filePath="+old,
	...
	function setText(path,oldName,newName){
		...
		var htmlText = '';
		if(path == ''){
			htmlText +='<input type="hidden" name="platform.photo" value=""></input>';
			htmlText +='<input type="hidden" name="platform.oldName" value=""></input>';
		}else{
			htmlText += '<div class="downlist">';
			htmlText +='<img src="core/common/images/downloadico.png" />';
			htmlText +='<ul>';
			htmlText +='<li><input readonly name="platform.oldName" value="'+oldName+'" onblur="loseFocus(this)"/>';
			htmlText +='</li><li>';
			//htmlText +='<a href="javascript:void(0)" onclick="">下载</a>';
			//htmlText +='<a href="javascript:void(0)" onclick="rename(this)">重命名</a>';
...
<body>
<form action="<%=basePath %>platform!update.action" method="post" name="form1" id="form1">
<input type="hidden" name="platform.contentText" id="contentText"/>
<input type="hidden" name="platform.id" value="${result.value.id }"/>
<input type="hidden" name="platform.kind" value="<%=ArticleKindEnum.LIST%>"/>
...
<div class="divlays" style="margin:0px;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
       <td class="layertdleft100"><span class="psred">*</span>平台名称：</td>
       <td class="layerright"><input name="platform.name" value="${result.value.name }" type="text" class="inputauto" /></td>
       <td class="layertdleft100"><span class="psred">*</span>平台标题：</td>
       <td class="layerright"><input name="platform.title" value="${result.value.title }" type="text" class="inputauto" /></td>
    </tr>
    <tr>
	   <td class="layertdleft100"><span class="psred"></span>平台URL：</td>
       <td class="layerright"><input name="platform.url" type="text" value="${result.value.url }" class="inputauto" /></td>
	   <td class="layertdleft100">自定义属性：</td>
       <td class="layerright">
          <input type="checkbox" name="platform.toped" value="<%=BooleanEnum.YES%>" <c:if test="${result.value.toped eq 'YES' }">checked="checked"</c:if> /> 置顶
          <input type="checkbox" name="platform.recommend" value="<%=BooleanEnum.YES%>" <c:if test="${result.value.recommend eq 'YES' }">checked="checked"</c:if> /> 首页推荐
		  <input type="checkbox" name="platform.bold" value="<%=BooleanEnum.YES%>" <c:if test="${result.value.bold eq 'YES' }">checked="checked"</c:if> /> 加粗
	   </td>
    </tr>
    <tr>
       <td class="layertdleft100"><span class="psred"></span>地址：</td>
       <td class="layerright"><input name="platform.address" type="text" value="${result.value.address }" class="inputauto" /></td>
	   <td class="layertdleft100">电话：</td>
       <td class="layerright"><input name="platform.phone" type="text" value="${result.value.phone }" class="inputauto" /></td>
    </tr>
    <tr>
    ...
    <tr>
    	<td class="layertdleft100">平台logo：</td>
    ...
    <tr>
       ...
       <td class="layerright" colspan="3" style="padding-top:1px;">
     		<textarea id=content name="platform.content" style="height:200px;margin-top:2px;" class="textareaauto">${result.value.content }</textarea>
			<script type="text/javascript">
