...
<script>
	...
	var rootMenu=[
		...
		if(CmsActivator.getHttpSessionService().isInResourceMap("cms_public_folder")){
		%>
		 	{text:"新建文件夹",
	...
	var docFolderMenu=[
		...
		<%
		boolean flag = false;
		if(CmsActivator.getHttpSessionService().isInResourceMap("cms_public_reName")){
			flag = true;
		%>
			{text:"重命名",
		...
		<%}%>
		<%if(CmsActivator.getHttpSessionService().isInResourceMap("cms_public_del")){
			if(flag){
		%>
		   ,
		<%}
		...
		<%}%>
		<%if(CmsActivator.getHttpSessionService().isInResourceMap("cms_public_move")){
			if(flag){
		%>
		   ,
		<%}
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td width="100%" valign="top">
		<div>
			<iframe src="parkmanager.cms/web/document/public_document_index.jsp" frameborder="0" id="msglist" width="100%"></iframe>
		</div>
	  </td>
    </tr>
  </table>
